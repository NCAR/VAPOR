...
class VDF_API NetCDFCFCollection : public NetCDFCollection {
  ...
  private:
    ...
    class DerivedVar_vertStag : public NetCDFCollection::DerivedVar {
      ...
      public:
        ...
        virtual bool TimeVarying() const {
            return (_ncdfc->IsTimeVarying(_unstagVar));
        };
        virtual std::vector<size_t> GetSpatialDims() const { return (_dims); }
        virtual std::vector<string> GetSpatialDimNames() const { return (_dimnames); }
        virtual size_t GetTimeDim() const {
            return (_ncdfc->GetTimeDim(_unstagVar));
        }
        virtual string GetTimeDimName() const {
            return (_ncdfc->GetTimeDimName(_unstagVar));
        };
        virtual bool GetMissingValue(double &mv) const {
            mv = 0.0;
            return (false);
    ...
    void _GetMissingValueMap(
        map<string, double> &missingValueMap, string &attname) const;
