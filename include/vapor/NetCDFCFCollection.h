...
private:
    ...
    public:
        DerivedVar_ocean_s_coordinate_g1(NetCDFCFCollection *ncdfcf, const std::map<string, string> &formula_map);
    ...
    public:
        DerivedVar_ocean_s_coordinate_g2(NetCDFCFCollection *ncdfcf, const std::map<string, string> &formula_map);
    ...
    private:
        ...
        virtual int DCZ2(const float *PS1, const float *PHIS1, float **TV2, const int NL, const float P0, float **HYBA, float **HYBB, const int KLEV, const int MLON, const int MLON2, float **HYPDLN,
                         float **HYALPH, float **PTERM, float **ZSLICE);
        std::vector<size_t> _dims;
        std::vector<string> _dimnames;
        size_t              _slice_num;
        float *             PS1;
    ...
    public:
        DerivedVar_noop(NetCDFCFCollection *ncdfcf, const std::map<string, string> &formula_map, string units);
        ...
        virtual std::vector<size_t> GetSpatialDims() const { return (_ncdfc->GetSpatialDims(_zvar)); };
        virtual std::vector<string> GetSpatialDimNames() const { return (_ncdfc->GetSpatialDimNames(_zvar)); };
        ...
        virtual bool GetMissingValue(double &mv) const { return (_ncdfc->GetMissingValue(_zvar, mv)); };
    ...
    public:
        DerivedVarTime(NetCDFCFCollection *ncdfcf, string native_var, string native_units, string derived_units);
