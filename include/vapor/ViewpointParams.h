...
public:
    ...
    //! world coordinates
    //
    void GetCameraViewDir(double viewDir[3]) const { getCurrentViewpoint()->GetCameraViewDir(viewDir); }
    ...
    //
    void GetRotationCenter(double center[3]) const { getCurrentViewpoint()->GetRotationCenter(center); }

    //! Specify rotation center in local coordinates
    //! \param[in] vector<double>& rotation center in local coordinates
    //! \retval int 0 if successful
    void SetRotationCenter(const double center[3]) { getCurrentViewpoint()->SetRotationCenter(center); }
    ...
    //
    virtual Transform *GetTransform(string dataSetName) const { return ((Transform *)_transforms->GetParams(dataSetName)); }
