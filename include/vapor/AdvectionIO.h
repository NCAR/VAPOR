...
auto OutputFlowlinesNumSteps(const Advection *adv, const char *filename, size_t numStep, bool append) -> int;

// Output trajectory to a maximum time.
// When `append == false`, a header will also be output.
// Otherwise, only trajectories are output.
auto OutputFlowlinesMaxTime(const Advection *adv, const char *filename, float maxTime, bool append) -> int;
