...
public:
    ...
    //!
    StructuredGrid(const size_t bs[3], const size_t min[3], const size_t max[3], const bool periodic[3], const std::vector<float *> &blks);
    ...
    //!
    StructuredGrid(const std::vector<size_t> &bs, const std::vector<size_t> &min, const std::vector<size_t> &max, const std::vector<bool> &periodic, const std::vector<float *> &blks);
    ...
    //! \deprecated This constructor is deprecated.
    //!
    StructuredGrid(const size_t bs[3], const size_t min[3], const size_t max[3], const bool periodic[3], const std::vector<float *> &blks, float missing_value);
    ...
    {
        for (int i = 0; i < 3; i++) min[i] = 0;
        for (int i = 0; i < _minabs.size(); i++) min[i] = _minabs[i];
    }
    ...
    {
        bdims.clear();
        for (int i = 0; i < _ndim; i++) bdims.push_back(_bdims[i]);
    }
    ...
    virtual int GetUserCoordinates(size_t i, size_t j, size_t k, double *x, double *y, double *z) const = 0;
    ...
    //!
    virtual void GetIJKIndex(double x, double y, double z, size_t *i, size_t *j, size_t *k) const = 0;
    ...
    public:
        ...
        int GetUserCoordinates(double *x, double *y, double *z) const { return (_rg->GetUserCoordinates(_x - _rg->_min[0], _y - _rg->_min[1], _rg->_ndim == 3 ? _z - _rg->_min[2] : 0, x, y, z)); };
...
private:
    ...
    void _StructuredGrid(const std::vector<size_t> &bs, const std::vector<size_t> &min, const std::vector<size_t> &max, const std::vector<bool> &periodic, const std::vector<float *> &blks);
};
};    // namespace VAPoR
#endif
