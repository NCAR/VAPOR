...
class VDF_API VDC : public VAPoR::DC {
  ...
  public:
    ...
    //
    virtual int CopyVar(
        DC &dc, size_t ts, string varname, int srclod, int dstlod) = 0;
    ...
    virtual bool CompressionInfo(
        vector<size_t> bs, string wname, size_t &nlevels, size_t &maxcratio) const = 0;
  ...
  protected:
    ...
    bool _valid_mask_var(
        ...
        vector<size_t> bs, bool compressed, string maskvar) const;
    ...
    bool _ValidCompressionBlock(
        vector<size_t> bs, string wname, vector<size_t> cratios) const;
