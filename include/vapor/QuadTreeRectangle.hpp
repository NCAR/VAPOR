...
class QuadTreeRectangle {
  public:
    ...
    bool Insert(T left, T top, T right, T bottom, S payload) {
        ...
        if (!root->intersects(rectangle_t(left, top, right, bottom)))
            return (false);
    ...
    //
    void GetStats(
        std::vector<size_t> &payload_histo, std::vector<size_t> &level_histo) const {
        payload_histo.clear();
        level_histo.clear();
  ...
  private:
    class rectangle_t {
      public:
        ...
        bool intersects(rectangle_t const &other) const {
            if (_left > other._right || _top > other._bottom)
                return (false);

            if (_right < other._left || _bottom < other._top)
                return (false);
            return (true);
        }
        ...
        bool contains(T x, T y) const {
            return ((_left <= x) && (_right >= x) && (_top <= y) && (_bottom >= y));
        }
        ...
        rectangle_t quadrant(uint32_t n) const {
            ...
            switch (n & 0x03) {
            case 0:
                return rectangle_t(_left, _top, center_x, center_y);
            case 1:
                return rectangle_t(center_x, _top, _right, center_y);
            case 2:
                return rectangle_t(_left, center_y, center_x, _bottom);
            case 3:
                return rectangle_t(center_x, center_y, _right, _bottom);
            }
    ...
    class node_t {
      public:
        node_t(int level = 0) : _level(level),
                                _is_leaf(true),
                                _child0(0),
                                _rectangle(0.0, 0.0, 1.0, 1.0) {}

        node_t(
            T left, T top, T right, T bottom, int level = 0) : _level(level),
                                                               _is_leaf(true),
                                                               _child0(0),
                                                               _rectangle(left, top, right, bottom) {}

        node_t(const rectangle_t &rec, int level = 0) : _level(level),
                                                        _is_leaf(true),
                                                        _child0(0),
                                                        _rectangle(rec) {}
        ...
        const node_t *quadrant(const std::vector<node_t *> &nodes, uint32_t n) const {
            ...
            case 1:
                return nodes[_child0 + 1];
            case 2:
                return nodes[_child0 + 2];
            case 3:
                return nodes[_child0 + 3];
            }
            return nodes[_child0 + 0];
            VAssert(0);
        }
        ...
        void get_payload_contains(
            ...
            T x, T y, std::vector<S> &payloads) const {
