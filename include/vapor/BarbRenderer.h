...
public:
    BarbRenderer(const ParamsMgr *pm, string winName, string dataSetName, string instName, DataStatus *ds);
...
private:
    ...
    int performRendering(const RenderParams *rParams, int actualRefLevel, float vectorScale, StructuredGrid *variableData[5]);
    ...
    void renderUnaligned(int rakeGrid[3], double rakeExts[6], StructuredGrid *variableData[5], int timestep, float vectorLengthScale, float rad, const RenderParams *params);

    float getHeightOffset(StructuredGrid *heightVar, float xCoord, float yCoord, bool &missing);

    void renderScottsGrid(int rakeGrid[3], double rakeExts[6], StructuredGrid *variableData[5], int timestep, float vectorLengthScale, float rad, const RenderParams *params);
    ...
    //! \param[in] int timestep:  current time step.
    //! \param[in] float rad Radius of barbs in voxel diameters.
    void renderAligned(int rakeGrid[3], double rakeExts[6], StructuredGrid *variableData[5], int timestep, float vectorLengthScale, float rad, const RenderParams *params);
