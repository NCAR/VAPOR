...
public:
    ...
    //! may have.
    //
    UnstructuredGrid(const std::vector<size_t> &node_dims, const std::vector<size_t> &cell_dims, const std::vector<size_t> &edge_dims, const std::vector<float *> &blks, const size_t *face_node_conn,
    ...
    {
        bdims.clear();
        for (int i = 0; i < _bdims.size(); i++) bdims.push_back(_bdims[i]);
    }
    ...
    //!
    virtual bool GetCellNeighbors(const std::vector<size_t> &cindices, std::vector<std::vector<size_t>> &cells) const;

    //! \copydoc Grid::GetNodeCells()
    //!
    virtual bool GetNodeCells(const std::vector<size_t> &indices, std::vector<std::vector<size_t>> &cells) const;
    ...
    public:
        ...
        ConstCoordItr &operator++()
        {    // ++prefix
            _impl->next();
            return *this;
        };
        ConstCoordItr operator++(int)
        {    // postfix++
        ...
        bool operator==(const ConstCoordItr &rhs) const { return (_impl->equal(rhs._impl->address())); }
    ...
    public:
        ...
        friend void swap(UnstructuredGrid::ForwardIterator<T> &a, UnstructuredGrid::ForwardIterator<T> &b)
        {
    ...
    ConstIterator cbegin(const std::vector<double> &minu, const std::vector<double> &maxu) const { return (ConstIterator(this, minu, maxu)); }
    ...
    public:
        ForwardCellIterator(T *sg, const std::vector<double> &minu = {}, const std::vector<double> &maxu = {});
        ...
        bool operator==(const ForwardCellIterator<T> &rhs) { return (_cellIndex == rhs._cellIndex); }
        bool operator!=(const ForwardCellIterator<T> &rhs) { return (!(*this == rhs)); }

        friend void swap(UnstructuredGrid::ForwardCellIterator<T> &a, UnstructuredGrid::ForwardCellIterator<T> &b)
        {
#ifdef DEAD
    ...
    //! inside or on the box defined by \p minu and \p maxu
    //
    CellIterator ConstCellBegin(const std::vector<double> &minu, const std::vector<double> &maxu) { return (CellIterator(this, minu, maxu)); }
#endif
    ...
    public:
        ForwardNodeIterator(T *sg, const std::vector<double> &minu = {}, const std::vector<double> &maxu = {});
        ...
        bool operator==(const ForwardNodeIterator<T> &rhs) { return (_nodeIndex == rhs._nodeIndex); }
        bool operator!=(const ForwardNodeIterator<T> &rhs) { return (!(*this == rhs)); }

        friend void swap(UnstructuredGrid::ForwardNodeIterator<T> &a, UnstructuredGrid::ForwardNodeIterator<T> &b)
        {
            std::swap(a._sg, b._sg);
            std::swap(a._dims, b._dims);
    ...
    //! inside or on the box defined by \p minu and \p maxu
    //
    NodeIterator ConstNodeBegin(const std::vector<double> &minu, const std::vector<double> &maxu) { return (NodeIterator(this, minu, maxu)); }
#endif
