...
VolumeIsoEventRouter::VolumeIsoEventRouter(QWidget *parent, ControlExec *ce) : RenderEventRouterGUI(ce, VolumeIsoParams::GetClassType())
{
    ...
    AddSubtab(
        "Appearance",
        new PGroup(
            {new PTFEditor(RenderParams::_variableNameTag, {PTFEditor::Histogram, PTFEditor::IsoValues}),
             ...
             (new PShowIf(VIP::_algorithmTag))
                 ->Equals(VolumeOSPRayIso::GetName())
                 ->Then({new PSection(
                     "OSPRay Parameters",
                     {
                 ...
                 ->Else({new PSection("Ray Tracing", {new PEnumDropdown(VIP::SamplingRateMultiplierTag, {"1x", "2x", "4x", "8x", "16x"}, {1, 2, 4, 8, 16}, "Sampling Rate Multiplier"),
                         ...
                         new PSection("Lighting", {new PCheckbox(VIP::LightingEnabledTag, "Enabled"), (new PDoubleSliderEdit(VIP::PhongAmbientTag, "Ambient"))->EnableDynamicUpdate(),
