...
bool Statistics::Update()
{
    ...
    if (enabledVars.size() > 0) {
        ...
        for (int i = 1; i < enabledVars.size(); i++) assert(numRefLevels == currentDmgr->GetNumRefLevels(enabledVars[i]));
        for (int i = 0; i < numRefLevels; i++) RefCombo->addItem(QString::number(i));
