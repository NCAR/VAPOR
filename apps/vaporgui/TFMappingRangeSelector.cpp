...
void TFMappingRangeSelector::_getDataRange(VAPoR::DataMgr *d, VAPoR::RenderParams *r, float *min, float *max) const
{
    ...
    d->GetDataRange(r->GetCurrentTimestep(), _getVariableName(), r->GetRefinementLevel(), r->GetCompressionLevel(), VAPoR::DataMgrUtils::GetDefaultMetaInfoStride(d, _getVariableName(), r->GetRefinementLevel()), range);
    *min = range[0];
    *max = range[1];
}
