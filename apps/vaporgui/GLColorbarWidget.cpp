...
: GLWidget(parent), _NUM_BINS(256), _parent(parent), _colormap(colormap), _texid(0), _texture(new unsigned char[_NUM_BINS * 4]), _updateTexture(true), _minValue(0.0), _maxValue(1.0)
{
    _minX = -0.03;
    _maxX = 1.03;
    _minY = -0.3;
...
void GLColorbarWidget::move(float dx, float, float)
{
    ...
    if (_colormap && _selected != NONE) {
        ...
        for (iter = _selectedCPs.begin(); iter != _selectedCPs.end(); iter++) { _colormap->move(*iter, dx * scaling); }
...
int GLColorbarWidget::paintGL()
{
    ...
    if (_colormap) {
        ...
        for (int i = 0; i < _colormap->numControlPoints(); i++) { drawControlPoint(i); }
...
list<float> GLColorbarWidget::selectedPoints()
{
    ...
    if (_colormap) {
        for (iter = _selectedCPs.begin(); iter != _selectedCPs.end(); iter++) { points.push_back(_colormap->controlPointValue(*iter)); }
    }
...
void GLColorbarWidget::newHsv(int h, int s, int v)
{
    if (_colormap) {
        if (_selectedCPs.size()) {
            ...
            for (iter = _selectedCPs.begin(); iter != _selectedCPs.end(); iter++) { _colormap->controlPointColor(*iter, ColorMap::Color(hf, sf, vf)); }
...
// space
//----------------------------------------------------------------------------
float GLColorbarWidget::dataToWorld(float x) { return (x - _parent->minDataValue()) / (_parent->maxDataValue() - _parent->minDataValue()); }

//----------------------------------------------------------------------------
// Transform the x position in the opengl world space into the data (model)
// space
//----------------------------------------------------------------------------
float GLColorbarWidget::worldToData(float x) { return _parent->minDataValue() + (x * (_parent->maxDataValue() - _parent->minDataValue())); }
