...
{
    AddSubtab("Variables", new PGroup({new PSection("Variable Selection", {new PScalarVariableSelector, new PColorMapVariableSelector}), new PFidelitySection}));
    ...
    AddSubtab(
        ...
        new PGroup(
            {(new PTFEditor)->ShowColormapBasedOnParam(VP::UseColormapVariableTag, false),
             ...
             (new PShowIf(VP::_algorithmTag))
                 ->Equals(VolumeOSPRay::GetName())
                 ->Then({new PSection(
                     "OSPRay Parameters",
                     {
                         ...
                         (new PDoubleInput(VolumeParams::OSPSampleRateScalar, "Volume Sample Rate Scalar"))
                             ->EnableBasedOnParam("osp_usePT", false)
                             ->SetTooltip("Scales the sampling rate along the ray throughout the volume. Increasing may significantly reduce performance."),
                     })})
                 ->Else({new PSection("Ray Tracing", {new PEnumDropdown(VP::SamplingRateMultiplierTag, {"1x", "2x", "4x", "8x", "16x"}, {1, 2, 4, 8, 16}, "Sampling Rate Multiplier"),
                                                      (new PDoubleSliderEdit(VP::VolumeDensityTag, "Volume Density"))
                                                          ->EnableDynamicUpdate()
                         ...
                         new PSection("Lighting",
                                      {new PCheckbox(VolumeParams::LightingEnabledTag, "Enabled"), (new PDoubleSliderEdit(VP::PhongAmbientTag, "Ambient"))->EnableDynamicUpdate(),
                                       (new PDoubleSliderEdit(VP::PhongDiffuseTag, "Diffuse"))->EnableDynamicUpdate(), (new PDoubleSliderEdit(VP::PhongSpecularTag, "Specular"))->EnableDynamicUpdate(),
