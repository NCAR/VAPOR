...
VolumeEventRouter::VolumeEventRouter(QWidget *parent, ControlExec *ce) : RenderEventRouterGUI(ce, VolumeParams::GetClassType())
{
    ...
    AddSubtab("Appearance",
              new PGroup({(new PTFEditor)->ShowColormapBasedOnParam(VP::UseColormapVariableTag, false),
                          new PSection("Ray Tracing", {new PStringDropdownHLI<VP>("Raytracing Algorithm", VP::GetAlgorithmNames(VP::Type::DVR), &VP::GetAlgorithm, &VP::SetAlgorithmByUser),
                          ...
                          new PSection("Lighting", {new PCheckbox(VolumeParams::LightingEnabledTag, "Enabled"), (new PDoubleSliderEdit(VP::PhongAmbientTag, "Ambient"))->EnableDynamicUpdate(),
