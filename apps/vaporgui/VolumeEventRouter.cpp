...
VolumeEventRouter::VolumeEventRouter(QWidget *parent, ControlExec *ce)
    ...
    AddSubtab("Variables", new PGroup({new PSection("Variable Selection", {new PScalarVariableSelector,
                                                                           new PColorMapVariableSelector}),
                                       new PFidelitySection}));

    AddSubtab("Appearance", new PGroup({(new PTFEditor)->ShowColormapBasedOnParam(VP::UseColormapVariableTag, false),
                                        ...
                                        (new PShowIf(VP::_algorithmTag))->Equals(VolumeOSPRay::GetName())->Then({new PSection("OSPRay Parameters", {
                                            ...
                                            ->Else({new PSection("Ray Tracing", {new PEnumDropdown(VP::SamplingRateMultiplierTag, {"1x", "2x", "4x", "8x", "16x"}, {1, 2, 4, 8, 16}, "Sampling Rate Multiplier"), (new PDoubleSliderEdit(VP::VolumeDensityTag, "Volume Density"))->EnableDynamicUpdate()->SetTooltip("Changes the overall density or 'opacity' of the volume allowing for finer tuning of the transfer function."), new PCheckbox(VP::UseColormapVariableTag, "Color by other variable"), (new PVariableSelector3D(RenderParams::_colorMapVariableNameTag))->ShowBasedOnParam(VP::UseColormapVariableTag)}), (new PColormapTFEditor)->ShowBasedOnParam(VolumeParams::UseColormapVariableTag), new PSection("Lighting", {new PCheckbox(VolumeParams::LightingEnabledTag, "Enabled"), (new PDoubleSliderEdit(VP::PhongAmbientTag, "Ambient"))->EnableDynamicUpdate(), (new PDoubleSliderEdit(VP::PhongDiffuseTag, "Diffuse"))->EnableDynamicUpdate(), (new PDoubleSliderEdit(VP::PhongSpecularTag, "Specular"))->EnableDynamicUpdate(), (new PDoubleSliderEdit(VP::PhongShininessTag, "Shininess"))->SetRange(1, 100)->EnableDynamicUpdate()})})}));
