...
FlowAppearanceSubtab::FlowAppearanceSubtab(QWidget *parent) : QVaporSubtab(parent)
{
    ...
    ps->Add(densityGroup);
    densityGroup->Add((new PDoubleSliderEdit(FlowParams::RenderDensityFalloffTag, "Density Falloff"))
                          ->SetRange(0.5, 10)
                          ->EnableDynamicUpdate()
...
void FlowSeedingSubtab::_createIntegrationSection()
{
    ...
    connect(_velocityMultiplierLineEdit, &VLineEdit_Deprecated::ValueChanged, this, &FlowSeedingSubtab::_velocityMultiplierChanged);
