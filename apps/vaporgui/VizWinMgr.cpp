...
void VizWinMgr::attachVisualizer(string vizName) {
	...
	_vizWindow[vizName] = new VizWin (
		MainForm::getInstance(), qvizname, vizName,
	...
	connect(
		...
		this, SLOT(vizAboutToDisappear(const string &))
	);
	connect(
		_vizWindow[vizName], SIGNAL(HasFocus(const string &)),
		this, SLOT(setActiveViz(const string &))
	);

	QMdiSubWindow* qsbw =
	MainForm::getInstance()->getMDIArea()->addSubWindow(_vizWindow[vizName]);
...
NavigationEventRouter* VizWinMgr::getViewpointRouter() {
	return ((NavigationEventRouter*)
		GetEventRouter(NavigationEventRouter::GetClassType())
	);
}
