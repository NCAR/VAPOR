...
VizWinMgr::VizWinMgr(ControlExec *ce)
{
    ...
    connect(this, SIGNAL(activateViz(const QString &)), _tabManager, SLOT(SetActiveViz(const QString &)));
}
...
void VizWinMgr::attachVisualizer(string vizName)
{
    ...
    _vizWindow[vizName] = new VizWin(MainForm::getInstance(), qvizname, vizName, _controlExec, m_trackBall

    );

    connect(_vizWindow[vizName], SIGNAL(Closing(const string &)), this, SLOT(vizAboutToDisappear(const string &)));
    connect(_vizWindow[vizName], SIGNAL(HasFocus(const string &)), this, SLOT(setActiveViz(const string &)));
...
void VizWinMgr::SetTrackBall(const double posvec[3], const double dirvec[3], const double upvec[3], const double centerRot[3], bool perspective)
{
    ParamsMgr *paramsMgr = _controlExec->GetParamsMgr();
...
RenderEventRouter *VizWinMgr::GetRenderEventRouter(string winName, string renderType, string instName) const
{
    map<string, EventRouter *>::const_iterator itr;
...
void VizWinMgr::registerEventRouter(const std::string tag, EventRouter *router) { _eventRouterMap[tag] = router; }
