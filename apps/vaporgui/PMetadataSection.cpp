...
PMetadataSection::PMetadataSection()
: PSection("Metadata", { new PMetadataTree() } )
{
}

PMetadataTree::PMetadataTree()
...
void PMetadataTree::updateGUI() const
{
    VAPoR::RenderParams* rp = dynamic_cast<VAPoR::RenderParams*>(getParams());
    if (rp == nullptr) MSG_ERR("Invalid RenderParams instance for acquiring variable metadata.");
    VAPoR::DataMgr* dm = getDataMgr();
    if (rp == nullptr) MSG_ERR("Invalid DataMgr instance for acquiring variable metadata.");
    ...
    for (auto qvar : qvars) {
        ...
        QString qDims = QString::number(dims[0]) + ":" + QString::number(dims[1]) + ":" + QString::number(dims[2]);
...
void PMetadataTree::_generateCoordVarInfo(QTreeWidgetItem* parent, const VAPoR::DC::DataVar dataVar) const {
    VAPoR::DataMgr* dm = getDataMgr();
    if (dm == nullptr) MSG_ERR("Invalid DataMgr instance for acquiring coordinate variable metadata.");
...
void PMetadataTree::_generateAttributeInfo(QTreeWidgetItem* parent, const VAPoR::DC::BaseVar baseVar) const {
