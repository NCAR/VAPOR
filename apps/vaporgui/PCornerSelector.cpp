...
PCornerSelector::PCornerSelector(std::string tag, std::string title)
: PLineItem(tag, title, _w = new QWidget)
{
...
//        l->setSpacing(0); This breaks the layout whereas setting H and V manually works... Thanks Qt.
    ...
    for (auto c : _checks)
        connect(c, &VCheckBox::ValueChanged, this, &PCornerSelector::checked);
}
...
std::vector<double> PCornerSelector::getValue() const
{
    return getParams()->GetValueDoubleVec(getTag());
}


void PCornerSelector::setValue(const std::vector<double> &v)
{
    getParams()->SetValueDoubleVec(getTag(), "", v);
}


PColorbarCornerSelector::PColorbarCornerSelector()
: PCornerSelector("", "Position") {}

std::vector<double> PColorbarCornerSelector::getValue() const
{
    return getParams<ColorbarPbase>()->GetCornerPosition();
}

void PColorbarCornerSelector::setValue(const std::vector<double> &v)
{
    getParams<ColorbarPbase>()->SetCornerPosition(v);
}
