...
PRegionSelector::PRegionSelector(const std::string &label)
: PSection(label)
{
...
PRegionSelector1D::PRegionSelector1D(int dim)
: PLineItem("", dim==0?"X":dim==1?"Y":"Z", _slider = new QRangeSliderTextCombo), _dim(dim)
{
...
VAPoR::Box *PRegionSelector1D::getBox() const
{
    return getParams<RenderParams>()->GetBox();
}
