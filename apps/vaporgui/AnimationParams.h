...
class PARAMS_API AnimationParams : public VAPoR::ParamsBase {
  ...
  public:
    ...
    size_t GetStartTimestep() const {
        return (size_t)GetValueLong(_startTimestepTag, 0);
    }
    ...
    size_t GetEndTimestep() const {
        return (size_t)GetValueLong(_endTimestepTag, (long)0);
    }
    ...
    bool GetPlayBackwards() const {
        return GetValueLong(_playBackwardsTag, 0);
    }
    ...
    size_t GetFrameStepSize() const {
        return (size_t)GetValueLong(_stepSizeTag, 1);
    }
    ...
    double GetMaxFrameRate() {
        return GetValueDouble(_maxRateTag, 1.0);
    }
    ...
    void SetMaxFrameRate(double rate) {
        SetValueDouble(_maxRateTag, "Set max frame rate", rate);
    }
    ...
    bool GetRepeating() const {
        return (GetValueLong(_repeatTag, false));
    }
    ...
    void SetRepeating(bool onOff) {
        SetValueLong(_repeatTag, "enable repeat play", (long)onOff);
    }
