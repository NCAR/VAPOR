...
void MappingFrame::populateSlicingHistogram(
    ...
    for (int j = 0; j < sampleRate; j++) {
        for (int i = 0; i < sampleRate; i++) {
            _getSampleCoordinates(
                coords, minExts, maxExts, i, j, orientation, sampleRate);
...
void MappingFrame::_getSampleCoordinates(
...
void MappingFrame::populateVolumetricHistogram(
    Grid *grid,
    std::vector<double> minExts,
    std::vector<double> maxExts) {
    float v;
