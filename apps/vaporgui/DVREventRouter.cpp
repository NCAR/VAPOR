...
DVREventRouter::DVREventRouter(QWidget *parent, VAPoR::ControlExec *ce) :
    QWidget(parent), ui(new Ui::DVREventRouter), RenderEventRouter( ce, DVREventRouter::GetClassType() )
{
    ui->setupUi(this);
}
...
std::string DVREventRouter::_getDescription() const
{
    return std::string("DVR Renderer"); 
}
 
std::string DVREventRouter::_getSmallIconImagePath() const
{
    return std::string("DVR_small.png");
}
 
std::string DVREventRouter::_getIconImagePath() const
{
    return std::string("DVR.png");
}
