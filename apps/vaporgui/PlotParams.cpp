...
void PlotParams::SetTimeExtents(vector<double> extents) { SetValueDoubleVec(_timeExtentsTag, "Spatial extents for time plotting", extents); }
...
void PlotParams::SetVarNames(vector<string> varNames)
{
    ...
    for (int i = 0; i < varNames.size(); i++) { cout << varNames[i] << " "; }
    cout << endl;
}
