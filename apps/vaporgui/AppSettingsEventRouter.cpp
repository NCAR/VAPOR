...
AppSettingsEventRouter::AppSettingsEventRouter(QWidget *parent, ControlExec *ce) : QWidget(parent), Ui_appSettingsTab(), EventRouter(ce, AppSettingsParams::GetClassType())
{
    setupUi(this);
...
void AppSettingsEventRouter::GetWebHelp(vector<pair<string, string>> &help) const
{
    ...
    help.push_back(make_pair("Overview of the AppSettings tab", "http://www.vapor.ucar.edu/docs/vapor-gui-help/appSettings-tab#AppSettingsOverview"));
}
...
void AppSettingsEventRouter::chooseAutoSaveFilename()
{
    ...
    QString            s = QFileDialog::getSaveFileName(this, "Choose a filename for autosaved session files", sParams->GetAutosaveName().c_str(), "Vapor Session File (*.vs3)");
    if (s != "") {
