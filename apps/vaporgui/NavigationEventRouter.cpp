...
void NavigationEventRouter::hookUpTab() {
	...
	connect (
		...
		this, SLOT(setCameraLatLonChanged())
	);
	connect (
...
void NavigationEventRouter::resizeProjTable() {
	datasetProjectionTable->horizontalHeader()->setSectionResizeMode(
		0, QHeaderView::Stretch
	);
	datasetProjectionTable->verticalHeader()->setSectionResizeMode(
		QHeaderView::Stretch
	);
...
void NavigationEventRouter::AlignView(int axis) {
	...
	bool status = _getViewpointParams(
		curCenter, curPosVec, curViewDir, curUpVec
	);
	if (! status)  return; 
...
void NavigationEventRouter::SetHomeViewpoint() {
	...
	paramsMgr->BeginSaveStateGroup("Set home viewpoint");
		...
		guiParams->SetValueDoubleVec(
			HomeModelViewMatrixTag, "Modelview matrix", m
		);
		guiParams->SetValueDoubleVec(
			HomeRotationCenterTag, "Camera rotation center", c
		);
	paramsMgr->EndSaveStateGroup();
}
...
void NavigationEventRouter::UseHomeViewpoint() {
	...
	vector <double> m = guiParams->GetValueDoubleVec(
		HomeModelViewMatrixTag, defaultV
	);

	vector <double> defaultC(3,0.0);
	vector <double> c = guiParams->GetValueDoubleVec(
		HomeRotationCenterTag, defaultC
	);
