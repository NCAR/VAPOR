...
FidelityWidget::FidelityWidget(QWidget* parent) 
    : QWidget(parent), Ui_FidelityWidgetGUI() 
{
    ...
    connect (
        ...
        SLOT(setNumRefinements(int))
    );
    connect (
        lodCombo,SIGNAL(activated(int)), this,
        SLOT(setCompRatio(int))
    );
    connect (
        fidelityDefaultButton, SIGNAL(clicked()), this,
        SLOT(SetFidelityDefault())
    );
    connect(
...
void FidelityWidget::getCmpFactors(
...
void FidelityWidget::Update( const DataMgr *dataMgr, 
                            ParamsMgr *paramsMgr,
