...
#define CreateInferredTemplateConstructor(className, T, constModifier)                                                        \
    template <class P>                                                                                                        \
    className##HLI<P> *new_##className##HLI(const std::string &label, T (P::*getter)() constModifier, void (P::*setter)(T)) { \
...
template <class PW, typename T, class P, typename... Args>
typename std::enable_if<std::is_same<T, bool>::value, T>::type class PWidgetHLI2 : public PWidgetHLI<PW, T, P, Args...> {
    virtual long getParamsLong() const override {
    }
};
