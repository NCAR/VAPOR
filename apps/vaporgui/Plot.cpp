...
Plot::Plot(QWidget *parent) : QDialog(parent), Ui_PlotWindow()
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (i < 3) { connect(_timeCheckBoxes[i], SIGNAL(stateChanged(int)), this, SLOT(constCheckboxChanged(int))); }
    }
}
...
void Plot::savePlotToFile()
{
    ...
    string file = fd->getSaveFileName(this, "Select Output File Name", "/", tr("(*.png)")).toStdString();
    ;
    delete fd;
...
//
int Plot::findNyquist(const vector<size_t> minv, const vector<size_t> maxv, const vector<double> minu, const vector<double> maxu, double &dX, double &dY, double &dZ) const
{
...
int Plot::getTemporalVectors(const vector<string> vars, map<string, vector<float>> &data, map<string, vector<float>> &iData) const
{
    ...
    vector<float> vec;
    for (size_t ts = ts0; ts <= ts1; ts++) { requestedTimes.push_back(allTimes[ts]); }
...
#ifdef FIXED
        if (_dm->GetVarType(_uVars[i]) == DataMgr::VAR3D) { varsAre2D = false; }
#endif
    }
