...
{
    vector<Observer *>::iterator it;
    for (it = _observers.begin(); it != _observers.end(); ++it) { (*it)->notify(); }
...
TimeSlider::TimeSlider(Range *range, QSlider *slider, int type) : MinMaxSlider(range, slider, type)
{
...
SinglePointSlider::SinglePointSlider(Range *range, QSlider *slider, int defaultPos) : Controller(range)
{
    _slider = slider;
    _increments = _slider->maximum() - _slider->minimum();
    _val = (_min + _max) / 2.f;
...
SinglePointLineEdit::SinglePointLineEdit(Range *range, QLineEdit *lineEdit, double defaultVal) : Controller(range)
{
    _lineEdit = lineEdit;
...
MinMaxLineEdit::MinMaxLineEdit(Range *range, QLineEdit *lineEdit, int type) : Controller(range, type)
{
    _lineEdit = lineEdit;
...
CenterSizeSlider::CenterSizeSlider(Range *range, QSlider *slider, int type) : Controller(range, type)
{
    _slider = slider;
    _increments = _slider->maximum() - _slider->minimum();
...
CenterSizeLineEdit::CenterSizeLineEdit(Range *range, QLineEdit *lineEdit, int type) : Controller(range, type)
{
    _lineEdit = lineEdit;
...
MinMaxTableCell::MinMaxTableCell(Range *range, QTableWidget *table, int type, int row, int col) : Controller(range, type)
{
    _table = table;
