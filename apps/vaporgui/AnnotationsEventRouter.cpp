...
AnnotationsEventRouter::AnnotationsEventRouter(
	...
	Ui_AnnotationsGUI(),
	EventRouter(ce, AnnotationsParams::GetClassType())
{
 ...
 ************************************************************/
void
AnnotationsEventRouter::hookUpTab()
{
	connect (
...
void AnnotationsEventRouter::GetWebHelp(
...
void AnnotationsEventRouter::scaleNormalizedCoordsToWorld(
...
void AnnotationsEventRouter::scaleWorldCoordsToNormalized(
	std::vector<double> &coords
) {
   std::vector<double> extents = getDomainExtents();
	for (int i=0; i<3; i++) {
...
void AnnotationsEventRouter::convertPCSToLon(
	double &xCoord) {
	double dummy = 0.; 
	convertPCSToLonLat(xCoord, dummy); 
}

void AnnotationsEventRouter::convertPCSToLat(
	double &yCoord) {
	double dummy = 0.; 
	convertPCSToLonLat(dummy, yCoord); 
}

void AnnotationsEventRouter::convertPCSToLonLat(
...
void AnnotationsEventRouter::convertLonToPCS(
	double &xCoord) {
	double dummy = 0.;
	convertLonLatToPCS(xCoord, dummy);
}

void AnnotationsEventRouter::convertLatToPCS(
	double &yCoord) {
	double dummy = 0.;
	convertLonLatToPCS(dummy, yCoord);
}

void AnnotationsEventRouter::convertLonLatToPCS(
	double &xCoord, double &yCoord
) {
	//ParamsMgr * paramsMgr= _controlExec->GetParamsMgr();
	//vector<string> names = paramsMgr->GetDataMgrNames();
...
void AnnotationsEventRouter::setColorHelper(
...
void AnnotationsEventRouter::updateColorHelper(
