...
QSliderEdit::QSliderEdit(QWidget *parent) : QWidget(parent),
                                            _ui(new Ui::QSliderEdit) {
    _ui->setupUi(this);

    _combo = new Combo(_ui->myLineEdit, _ui->mySlider);
    _combo->SetPrecision(5);

    connect(_combo, SIGNAL(valueChanged(double)),
            this, SLOT(_comboValueChanged(double)));
    connect(_combo, SIGNAL(valueChanged(int)),
            this, SLOT(_comboValueChanged(int)));
}
...
void QSliderEdit::SetLabel(const QString &text) {
...
void QSliderEdit::_comboValueChanged(int val) {
    emit valueChanged(val);
}

void QSliderEdit::SetIntType(bool val) {
    _combo->SetIntType(val);
}
