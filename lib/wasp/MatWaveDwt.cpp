...
template <class T>
int valid_float(T *a, size_t n, bool invalid_float_abort) {
...
void inverse_xform_even(
    const double *cA, const double *cD, size_t sigOutLen,
    const double *low_filter, const double *high_filter,
...
void inverse_xform(
    ...
    if (filterLen % 2) {
        inverse_xform_odd(
            cA, cD, sigOutLen, low_filter, high_filter, filterLen, sigOut);
    } else {
        inverse_xform_even(
            cA, cD, sigOutLen, low_filter, high_filter, filterLen, sigOut,
            matlab);
...
MatWaveDwt::MatWaveDwt(
    const string &wname, const string &mode) : MatWaveBase(wname, mode) {
}
...
int dwt_template(
    ...
    U *cA, U *cD, size_t L[3], SmartBuf &sbuf, V dummy) {
    if (!wf) {
    ...
    int rc = wextend_1D_center(
        sigIn, sigInLen, sigExtended, extendLen, mode, mode);
    if (rc < 0)
        return (-1);
    ...
    if (!std::numeric_limits<V>::is_integer) {
        int rc = valid_float(
            sigExtended, sigExtendedLen, dwt->InvalidFloatAbortOnOff());
        if (rc < 0)
            return (-1);
    }
    ...
    if (!std::numeric_limits<V>::is_integer) {
        ...
        forward_xform(
            s, L[0] + L[1], wf->GetLowDecomFilCoef(),
            wf->GetHighDecomFilCoef(), filterLen,
...
int MatWaveDwt::dwt(
    const long *sigIn, size_t sigInLen, long *C, size_t L[3]) {
    long *cA = C;
...
int MatWaveDwt::dwt(
    const int *sigIn, size_t sigInLen, int *C, size_t L[3]) {
    int *cA = C;
...
int MatWaveDwt::dwt(
    const long *sigIn, size_t sigInLen, long *cA, long *cD, size_t L[3]) {
    long dummy = 0;
...
int MatWaveDwt::dwt(
    const int *sigIn, size_t sigInLen, int *cA, int *cD, size_t L[3]) {
    long dummy = 0;
...
int idwt_template(
    ...
    V *buf = (V *)sbuf.Alloc(
        sizeof(dummy) * (cATempLen + cDTempLen + reconTempLen + cDPadLen));
    ...
    if (do_sym_conv) {
        int rc = wextend_1D_center(
            cA, L[0], cATemp, extendLen, cALeftMode, cARightMode);
        if (rc < 0)
            return (-1);
    ...
    if (!std::numeric_limits<V>::is_integer) {
        int rc = valid_float(
            cATemp, cATempLen, dwt->InvalidFloatAbortOnOff());
        if (rc < 0)
            return (-1);

        rc = valid_float(
            cDTemp, cDTempLen, dwt->InvalidFloatAbortOnOff());
        if (rc < 0)
            return (-1);
    }
    ...
    if (!std::numeric_limits<V>::is_integer) {
        ...
        inverse_xform(
            cAdbl, cDdbl, L[2], wf->GetLowReconFilCoef(),
            wf->GetHighReconFilCoef(), filterLen, s, !do_sym_conv);
    } else {
        const WaveFiltInt *wfi = dynamic_cast<const WaveFiltInt *>(wf);
        assert(wfi != NULL);
...
int MatWaveDwt::idwt(
    const long *C, const size_t L[3], long *sigOut) {
    const long *cA = C;
    const long *cD = C + L[0];
    long dummy = 0;
...
int MatWaveDwt::idwt(
    const int *C, const size_t L[3], int *sigOut) {
    const int *cA = C;
    const int *cD = C + L[0];
    long dummy = 0;
...
int MatWaveDwt::idwt(
    const long *cA, const long *cD, const size_t L[3], long *sigOut) {
    long dummy = 0;
...
int MatWaveDwt::idwt(
    const int *cA, const int *cD, const size_t L[3], int *sigOut) {
    long dummy = 0;
...
int dwt2d_template(
    ...
    SmartBuf &sbuf2d, SmartBuf &sbuf1d, V dummy) {
    if (!wf) {
        MatWaveDwt::SetErrMsg("Invalid state, no wavelet");
        return (-1);
    }
    ...
    V *buf2d = (V *)sbuf2d.Alloc(
        sizeof(dummy) * (passXLen + transposeLen + passYLen));
...
int idwt2d_template(
    ...
    V *buf2d = (V *)sbuf2d.Alloc(
        sizeof(dummy) * (passYLen + transposeLen + passXLen));
...
int MatWaveDwt::idwt2d(
    const long *C, const size_t L[10], long *sigOut) {
    const long *cA = C;
...
int _dwtz_template(
    MatWaveDwt *dwt,
    V *sigIn, size_t sigInX, size_t sigInY, size_t sigInZ,
    const WaveFiltBase *wf,
    MatWaveBase::dwtmode_t mode,
    T *cA, T *cD,
    SmartBuf &sbuf3d, SmartBuf &sbuf1d,
    V dummy) {
    if (!wf) {
        MatWaveDwt::SetErrMsg("Invalid state, no wavelet");
        return (-1);
...
int _idwtz_template(
    ...
    const WaveFiltBase *wf, MatWaveBase::dwtmode_t mode, V *sigOut,
    size_t sigOutZ,
    SmartBuf &sbuf3d, SmartBuf &sbuf1d,
    V dummy) {
    if (!wf) {
...
int MatWaveDwt::idwt3d(
    const long *C, const size_t L[27], long *sigOut) {
    const long *cLLL = C;
...
int MatWaveDwt::idwt3d(
    const int *C, const size_t L[27], int *sigOut) {
