...
bool DataMgr::_hasVerticalConversion(
    string varname, string &standard_name, string &formula_terms) const {
    standard_name.clear();
    formula_terms.clear();
...
bool DataMgr::_get_coord_vars(
    string varname, vector<DC::CoordVar> &scvarsinfo, DC::CoordVar &tcvarinfo) const {
    scvarsinfo.clear();
...
bool DataMgr::_getDataVarInfoDerived(
    string varname, VAPoR::DC::DataVar &varInfo) const {
...
bool DataMgr::_getCoordVarInfoDerived(
    string varname, VAPoR::DC::CoordVar &varInfo) const {
...
bool DataMgr::_getBaseVarInfoDerived(
    string varname, VAPoR::DC::BaseVar &varInfo) const {
...
bool DataMgr::_getVarDimNames(
    string varname, vector<string> &dimnames) const {
    dimnames.clear();
    ...
    for (int i = 0; i < varnames.size(); i++) {
        ...
        DerivedCoordVarStandardWRF_Terrain *derivedVar =
            new DerivedCoordVarStandardWRF_Terrain(
                derivedCoordvars[0], _dc, formula_terms);
