...
void DCMPAS::_addMissingFlag(int *data) const
{
    ...
    for (size_t j = 0; j < nCells; j++) {
        for (int i = nEdgesOnCell[j]; i < nMaxEdges; i++) { data[j + i * nMaxEdges] = -1; }
    }
}
...
int DCMPAS::Read(int *data)
{
    ...
    // Special handling for some auxiliary variables
    //
    if (_ovr_varname == verticesOnCellVarName) { _addMissingFlag(data); }

    if (is_connectivity_var(_ovr_varname)) { _splitOnBoundary(_ovr_varname, data); }
...
int DCMPAS::_InitCoordvars(NetCDFCollection *ncdfc)
{
    ...
    _coordVarsMap[name] = CoordVar(name, units, DC::FLOAT, periodic, axis, false, dimnames, vector<size_t>(), timeDimName);
...
int DCMPAS::_InitHorizontalCoordinatesDerived(NetCDFCollection *ncdfc)
{
    ...
    for (int i = 0; i < coordpairs.size(); i++) {
        ...
        derivedX = new DerivedVarHorizontal(ncdfc, coordpairs[i].first, coordpairs[i].second, _proj4API, true, true, false);
        ...
        derivedY = new DerivedVarHorizontal(ncdfc, coordpairs[i].first, coordpairs[i].second, _proj4API, false, true, false);
...
int DCMPAS::_InitDataVars(NetCDFCollection *ncdfc)
{
    ...
    for (int i = 0; i < vars.size(); i++) {
        ...
        if (!has_missing) {
            _dataVarsMap[vars[i]] = DataVar(vars[i], units, DC::FLOAT, periodic, meshname, vector<size_t>(), time_coordvar, DC::Mesh::NODE);
        } else {
            _dataVarsMap[vars[i]] = DataVar(vars[i], units, DC::FLOAT, periodic, meshname, vector<size_t>(), time_coordvar, DC::Mesh::NODE, mv);
        }
    }
...
//
DCMPAS::DerivedVarHorizontal::DerivedVarHorizontal(NetCDFCollection *ncdfc, string lonname, string latname, Proj4API *proj4API, bool xflag, bool uGridFlag, bool degrees) : DerivedVar(ncdfc)
{
    _lonname = lonname;
    _latname = latname;
    _xflag = xflag;
