...
DC::XType DCWRF::GetAttType(string varname, string attname) const
{
    ...
    case NC_DOUBLE: return XType::DOUBLE; break;
    case NC_INT: return XType::INT32; break;
    case NC_INT64: return XType::INT64; break;
    case NC_CHAR: return XType::TEXT; break;
    default: return XType::INVALID;
    }
}
...
int DCWRF::ReadRegionBlock(const vector<size_t> &min, const vector<size_t> &max, float *region) { return (DCWRF::ReadRegion(min, max, region)); }
...
{
    if (ts >= _timeLookup.size()) { return (false); }
    ts = _timeLookup[ts];
...
DCWRF::DerivedVarHorizontal *DCWRF::_InitHorizontalCoordinatesHelper(NetCDFCollection *ncdfc, Proj4API *proj4API, string name, string fastest_dim, int axis, vector<size_t> latlondims)
{
    ...
    _coordVarsMap[name] = CoordVar(name, "meters", DC::FLOAT, periodic, axis, true, sdimnames, bs, time_dim_name);
...
int DCWRF::_InitHorizontalCoordinates(NetCDFCollection *ncdfc, Proj4API *proj4API)
{
    ...
    //
    _derivedY = _InitHorizontalCoordinatesHelper(ncdfc, proj4API, "Y", "south_north", 1, latlondims);

    // "XU" coordinate, staggered
    //
    _derivedXU = _InitHorizontalCoordinatesHelper(ncdfc, proj4API, "XU", "west_east_stag", 0, latlondims);

    // "YU" coordinate, unstaggered
    //
    _derivedYU = _InitHorizontalCoordinatesHelper(ncdfc, proj4API, "YU", "south_north", 1, latlondims);

    // "XV" coordinate, unstaggered
    //
    _derivedXV = _InitHorizontalCoordinatesHelper(ncdfc, proj4API, "XV", "west_east", 0, latlondims);

    // "YV" coordinate, staggered
    //
    _derivedYV = _InitHorizontalCoordinatesHelper(ncdfc, proj4API, "YV", "south_north_stag", 1, latlondims);

    return (0);
}

DCWRF::DerivedVarElevation *DCWRF::_InitVerticalCoordinatesHelper(NetCDFCollection *ncdfc, string name, vector<string> sdimnames)
{
    ...
    _coordVarsMap[name] = CoordVar(name, "meters", DC::FLOAT, periodic, 2, true, sdimnames, bs, time_dim_name);
...
int DCWRF::_InitVerticalCoordinates(NetCDFCollection *ncdfc)
{
    ...
    //
    if (_dimsMap.find("bottom_top") == _dimsMap.end()) return (0);
    if (_dimsMap.find("bottom_top_stag") == _dimsMap.end()) return (0);
...
int DCWRF::_InitTime(NetCDFCollection *ncdfc)
{
    ...
    for (int i = 0; i < times.size(); i++) { timepairs.push_back(make_pair(i, times[i])); }
    sort(timepairs.begin(), timepairs.end(), mycompare);

    for (int i = 0; i < timepairs.size(); i++) { _timeLookup.push_back(timepairs[i].first); }
    ...
    vector<bool> periodic(1, false);
    _coordVarsMap[name] = CoordVar(name, "seconds", DC::FLOAT, periodic, 3, false, vector<string>(), vector<size_t>(), "Time");
...
)
{
    ...
    if (dimnames[0].compare("west_east") == 0 && dimnames[1].compare("south_north") == 0) {
        ...
        if ((dimnames.size() > 2) && dimnames[2].compare("bottom_top") == 0) { scoordvars.push_back("ELEVATIONU"); }
        ...
        if ((dimnames.size() > 2) && dimnames[2].compare("bottom_top") == 0) { scoordvars.push_back("ELEVATIONV"); }
    } else {
        return (false);
    }
...
//
DCWRF::DerivedVarElevation::DerivedVarElevation(NetCDFCollection *ncdfc, string name, const vector<DC::Dimension> &dims, float grav) : DerivedVar(ncdfc)
{
...
int DCWRF::DerivedVarElevation::Read(float *buf, int)
{
    ...
    int    rc;
    float *ptr = buf;
    while ((rc = DCWRF::DerivedVarElevation::ReadSlice(ptr, 0)) > 0) { ptr += nx * ny; }
    return (rc);
...
int DCWRF::DerivedVarElevation::_ReadSlice(float *slice)
{
    ...
    if (_firstSlice && _zinterpolate) {
        ...
        for (size_t y = 0; y < ny; y++) {
            for (size_t x = 0; x < nx; x++) { _zsliceBuf[y * nx + x] = (_PH[y * nx + x] + _PHB[y * nx + x]) / _grav; }
        }
        _firstSlice = false;
    }
    int rc;
    ...
    if (_zinterpolate) {
        ...
        for (size_t y = 0; y < ny; y++) {
            for (size_t x = 0; x < nx; x++) { slice[y * nx + x] = (_PH[y * nx + x] + _PHB[y * nx + x]) / _grav; }
        }
    }
    return (1);
}
...
int DCWRF::DerivedVarElevation::ReadSlice(float *slice, int)
{
    ...
    // required.
    //
    if (!(_xextrapolate || _yextrapolate)) { return (DerivedVarElevation::_ReadSlice(slice)); }
    ...
    if (_xextrapolate) {
        ...
        for (size_t y = 0; y < ny; y++) {
            for (size_t x = 1; x < nxs - 1; x++) { slice[y * nxs + x] = 0.5 * (_xysliceBuf[y * nx + x - 1] + _xysliceBuf[y * nx + x]); }
        }
        ...
        for (size_t y = 0; y < ny; y++) {    // left boundary
            slice[y * nxs] = _xysliceBuf[y * nx + 0] + (-0.5 * (_xysliceBuf[y * nx + 1] - _zsliceBuf[y * nx + 0]));
        }
        for (size_t y = 0; y < ny; y++) {    // right boundary
            slice[y * nxs + nxs - 1] = _xysliceBuf[y * nx + nx - 2] + (0.5 * (_xysliceBuf[y * nx + nx - 1] - _zsliceBuf[y * nx + nx - 2]));
        }
    } else {
        // First interpolate interior points
        //
        ...
        for (size_t y = 1; y < nys - 1; y++) {
            for (size_t x = 0; x < nx; x++) { slice[y * nx + x] = 0.5 * (_xysliceBuf[(y - 1) * nx + x] + _xysliceBuf[y * nx + x]); }
        }
        ...
        for (size_t x = 0; x < nx; x++) {    // top boundary
            slice[(nys - 1) * nx + x] = _xysliceBuf[(ny - 2) * nx + x] + (0.5 * (_xysliceBuf[(ny - 1) * nx + x] - _zsliceBuf[(ny - 2) * nx + x]));
        }
    }
...
//
DCWRF::DerivedVarHorizontal::DerivedVarHorizontal(NetCDFCollection *ncdfc, string name, const vector<DC::Dimension> &dims, const vector<size_t> latlondims, Proj4API *proj4API) : DerivedVar(ncdfc)
{
    ...
    //
    _hasStagCVars = true;
    if (!ncdfc->VariableExists(0, "XLONG_U")) _hasStagCVars = false;
    if (!ncdfc->VariableExists(0, "XLONG_V")) _hasStagCVars = false;
    if (!ncdfc->VariableExists(0, "XLAT_U")) _hasStagCVars = false;
    if (!ncdfc->VariableExists(0, "XLAT_V")) _hasStagCVars = false;
...
int DCWRF::DerivedVarHorizontal::_GetCartCoords(size_t ts)
{
    ...
    if (_name.compare(0, 1, "X") == 0) {
        ...
        for (int i = 0; i < _ncoords; i++) { _coords[i] = minx + (float)i * delta; }
    } else {
        assert(_ncoords == _ny);
        delta = (maxy - miny) / (float)(_ny - 1);
        for (int i = 0; i < _ncoords; i++) { _coords[i] = miny + (float)i * delta; }
    }
...
int DCWRF::DerivedVarHorizontal::Read(float *buf, int)
{
    ...
    //
    bool staggered = ((_name.compare("XU") == 0) || (_name.compare("YU") == 0) || (_name.compare("XV") == 0) || (_name.compare("YV") == 0));

    if (!staggered || _hasStagCVars) {
        for (int i = 0; i < _ncoords; i++) { buf[i] = _coords[i]; }
    } else {
        // Need to linearly interpolate staggered coordinates
        // from non-staggered coords
        //
        buf[0] = _coords[0] - ((_coords[1] - _coords[0]) * 0.5);
        for (int i = 1; i < _ncoords; i++) { buf[i] = (_coords[i - 1] + _coords[i]) * 0.5; }
        buf[_ncoords] = _coords[_ncoords - 1] + ((_coords[_ncoords - 1] - _coords[_ncoords - 2]) * 0.5);
    }
    return (0);
}
...
//
DCWRF::DerivedVarTime::DerivedVarTime(NetCDFCollection *ncdfc, DC::Dimension dim, const std::vector<float> &timecoords) : DerivedVar(ncdfc)
{
    _ts = 0;
