...
void LayeredGrid::GetIndicesFloor(
	const std::vector <double> &coords,
	std::vector <size_t> &indices
) const {
...
bool LayeredGrid::InsideGrid(const std::vector <double> &coords) const {
	...
	double iwgt, jwgt;
	if (x1!=x0) iwgt = fabs((clampedCoords[0]-x0) / (x1-x0));
		else iwgt = 0.0;

	if (y1!=y0) jwgt = fabs((clampedCoords[1]-y0) / (y1-y0));
		else jwgt = 0.0;
...
bool LayeredGrid::_bsearchKIndexFloor(
