...
void LayeredGrid::_layeredGrid(const vector<bool> &periodic, const vector<double> &minu, const vector<double> &maxu, const RegularGrid &rg)
{
...
LayeredGrid::LayeredGrid(const vector<size_t> &bs, const vector<size_t> &min, const vector<size_t> &max, const vector<double> &minu, const vector<double> &maxu, const vector<bool> &periodic,
...
#ifdef DEAD
LayeredGrid::LayeredGrid(const size_t bs[3], const size_t min[3], const size_t max[3], const double extents[6], const bool periodic[3], const vector<float *> &blks, const RegularGrid &rg)
: RegularGrid(bs, min, max, periodic, blks)
{
    ...
    for (int i = 0; i < _ndim; i++) { periodic_v.push_back(periodic[i]); }
...
int LayeredGrid::GetUserCoordinates(size_t i, size_t j, size_t k, double *x, double *y, double *z) const
{
...
void LayeredGrid::GetIJKIndexFloor(double x, double y, double z, size_t *i, size_t *j, size_t *k) const
{
    // First get ijk index of non-varying dimensions
    // N.B. index returned for varying dimension is bogus
    //
...
void LayeredGrid::_getBilinearWeights(double x, double y, double z, double &iwgt, double &jwgt) const
{
    size_t dims[3];
    GetDimensions(dims);
...
double LayeredGrid::_interpolateVaryingCoord(size_t i0, size_t j0, size_t k0, double x, double y, double z) const
{
