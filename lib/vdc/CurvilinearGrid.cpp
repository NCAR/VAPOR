...
void CurvilinearGrid::_curvilinearGrid(const RegularGrid &xrg, const RegularGrid &yrg, const vector<double> &zcoords, const KDTreeRG *kdtree)
{
    _zcoords.clear();
    _minu.clear();
    _maxu.clear();
...
void CurvilinearGrid::GetUserCoordinates(const std::vector<size_t> &indices, std::vector<double> &coords) const
{
    ...
    if (GetTopologyDim() > 2) { coords.push_back(_zcoords[cIndices[2]]); }
}
...
CurvilinearGrid::ConstCoordItrCG::ConstCoordItrCG(const CurvilinearGrid *cg, bool begin) : ConstCoordItrAbstract()
{
    ...
    if (dims.size() == 3) { _coords.push_back(_cg->_zcoords[0]); }
}
