...
float *Grid::GetValueAtIndex(
    const std::vector<float *> &blks,
    const size_t indices[3]) const {
...
void Grid::_getUserCoordinatesHelper(
    const vector<double> &coords, double &x, double &y, double &z) const {
    if (coords.size() >= 1) {
        x = coords[0];
    }
    if (coords.size() >= 2) {
...
Grid::ConstNodeIteratorSG::ConstNodeIteratorSG(
    const Grid *g, bool begin) : ConstNodeIteratorAbstract() {
...
Grid::ConstCellIteratorSG::ConstCellIteratorSG(
    const Grid *g, bool begin) : ConstCellIteratorAbstract() {
...
Grid::ForwardIterator<T>::ForwardIterator(
    T *rg, bool begin,
    const vector<double> &minu, const vector<double> &maxu) : _pred(minu, maxu) {
