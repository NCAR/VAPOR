...
void DC::Mesh::_Mesh(string name, std::vector<string> coord_vars, int max_nodes_per_face, Mesh::Type mtype)
{
    _name.clear();
    _dim_names.clear();
    _coord_vars.clear();
...
DC::Mesh::Mesh(std::string name, size_t max_nodes_per_face, std::string node_dim_name, std::string face_dim_name, std::vector<std::string> coord_vars, std::string face_node_var)
{
...
DC::Mesh::Mesh(std::string name, size_t max_nodes_per_face, std::string node_dim_name, std::string face_dim_name, std::string layers_dim_name, std::vector<std::string> coord_vars,
...
vector<string> DC::GetDataVarNames(int ndim, bool spatial) const
{
    ...
    for (int i = 0; i < allnames.size(); i++) {
        ...
        if (nsdim == ndim) { names.push_back(allnames[i]); }
    }
    return (names);
}
...
vector<string> DC::GetCoordVarNames(int ndim, bool spatial) const
{
    ...
    for (int i = 0; i < allnames.size(); i++) {
        ...
        //
        if (!spatial && !cvar.GetTimeDimName().empty()) { myndim++; }

        if (myndim == ndim) { names.push_back(allnames[i]); }
    }
    return (names);
}
...
bool DC::GetNumDimensions(string varname, size_t &ndim) const
{
    ...
    if (status) {
        ...
        case Mesh::UNSTRUC_2D: ndim = 2; break;
        case Mesh::UNSTRUC_LAYERED: ndim = 3; break;
        case Mesh::UNSTRUC_3D: ndim = 3; break;
        case Mesh::STRUCTURED: ndim = m.GetDimNames().size(); break;
        default: ndim = 0;
        }
        return (true);
    }
