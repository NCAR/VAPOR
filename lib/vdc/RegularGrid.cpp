...
RegularGrid::RegularGrid(
    ...
    const vector<float *> &blks) : StructuredGrid(bs, min, max, periodic, blks) {
...
RegularGrid::RegularGrid(
    ...
    float missing_value) : StructuredGrid(bs, min, max, periodic, blks, missing_value) {
...
void RegularGrid::GetBoundingBox(
    ...
    RegularGrid::GetUserCoordinates(
        a_min[0], a_min[1], a_min[2], &(a_minu[0]), &(a_minu[1]), &(a_minu[2]));
    RegularGrid::GetUserCoordinates(
        a_max[0], a_max[1], a_max[2], &(a_maxu[0]), &(a_maxu[1]), &(a_maxu[2]));
...
void RegularGrid::GetBoundingBox(
    const size_t min[3], const size_t max[3], double extents[6]) const {
    for (int i = 0; i < 6; i++)
        extents[i] = 0.0;
...
void RegularGrid::GetEnclosingRegion(
    ...
    RegularGrid::GetIJKIndex(
        a_minu[0], a_minu[1], a_minu[2], &temp_min[0], &temp_min[1], &temp_min[2]);
    RegularGrid::GetIJKIndex(
        a_maxu[0], a_maxu[1], a_maxu[2], &temp_max[0], &temp_max[1], &temp_max[2]);
