...
StructuredGrid::ConstNodeIteratorRG::ConstNodeIteratorRG(const StructuredGrid *rg, bool begin) : ConstNodeIteratorAbstract()
{
...
StructuredGrid::ConstNodeIteratorRG::ConstNodeIteratorRG(const StructuredGrid *rg, const std::vector<double> &minu, const std::vector<double> &maxu) : ConstNodeIteratorAbstract()
{
    assert(0 && "Not supported");
}

StructuredGrid::ConstNodeIteratorRG::ConstNodeIteratorRG(const ConstNodeIteratorRG &rhs) : ConstNodeIteratorAbstract()
{
    _dims = rhs._dims;
...
{
    _nodeIndex[0]++;
    if (_nodeIndex[0] < _dims[0]) { return; }

    _nodeIndex[0] = 0;
    _nodeIndex[1]++;
    if (_nodeIndex[1] < _dims[1] || _dims.size() == 2) { return; }

    _nodeIndex[1] = 0;
    _nodeIndex[2]++;
    if (_nodeIndex[2] < _dims[2] || _dims.size() == 3) { return; }
