    ...
    for (size_t k = 0; k < nz; k++) {
        for (size_t j = 0; j < ny; j++) {
            for (size_t i = 0, ii = i0; i < nx - 1; i++, ii++) {
                src[k * nxs * ny + j * nxs + ii] = 0.5 * (buf[k * nx * ny + j * nx + i] + buf[k * nx * ny + j * nx + i + 1]);
            }
        }
    }
...
DerivedCoordVar_PCSFromLatLon::DerivedCoordVar_PCSFromLatLon(
    ...
    bool lonFlag) : DerivedCoordVar(derivedVarName) {
...
DerivedCoordVar_CF1D::DerivedCoordVar_CF1D(
    ...
    string dimName, int axis, string units) : DerivedCoordVar(derivedVarName) {
...
DerivedCoordVar_CF2D::DerivedCoordVar_CF2D(
    ...
    const vector<float> &data) : DerivedCoordVar(derivedVarName) {
...
DerivedCoordVar_Time::DerivedCoordVar_Time(
    string derivedVarName, string dimName, size_t n) : DerivedCoordVar(derivedVarName) {

    _times.clear();
    for (size_t i = 0; i < n; i++)
        _times.push_back((float)i);
...
int DerivedCoordVar_Time::Initialize() {
    return (0);
}

bool DerivedCoordVar_Time::GetBaseVarInfo(
    DC::BaseVar &var) const {
    var = _coordVarInfo;
    return (true);
}

bool DerivedCoordVar_Time::GetCoordVarInfo(
    DC::CoordVar &cvar) const {
    cvar = _coordVarInfo;
    return (true);
}

int DerivedCoordVar_Time::GetDimLensAtLevel(
    int level, std::vector<size_t> &dims_at_level,
    std::vector<size_t> &bs_at_level) const {
    dims_at_level.clear();
    bs_at_level.clear();
...
int DerivedCoordVar_Time::OpenVariableRead(
    size_t ts, int level, int lod) {
    ts = ts < _times.size() ? ts : _times.size() - 1;
...
int DerivedCoordVar_Time::ReadRegion(
    int fd,
    const vector<size_t> &min, const vector<size_t> &max, float *region) {
    VAssert(min.size() == 0);
    VAssert(max.size() == 0);
...
bool DerivedCoordVar_Time::VariableExists(
    size_t ts,
    int reflevel,
    int lod) const {
...
DerivedCoordVarStandardWRF_Terrain::DerivedCoordVarStandardWRF_Terrain(
    DC *dc, string mesh, string formula) : DerivedCFVertCoordVar("", dc, mesh, formula) {
...
int DerivedCoordVarStandardAHSPC::initialize_missing_values() {
    ...
    if (dataInfo.GetHasMissing())
        _psVarMV = dataInfo.GetMissingValue();
