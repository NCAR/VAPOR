...
template<>
int BOVCollection::_readMetadata<DC::XType>( const std::string &token, std::string &line, DC::XType &value, bool verbose ) {
    // Skip comments
    if (line[0] == '#') {
        return 0;
    }
    ...
    if( pos != std::string::npos ) {  // We found the token
        ...
        if (verbose) {
            std::cout << std::setw(20) << token << " " << _dataFormat << std::endl;
        }
...
template<typename T>
int BOVCollection::_readMetadata( const std::string &token, std::string &line, T &value, bool verbose ) {
    // Skip comments
    if (line[0] == '#') {
        return 0;
    }
    ...
    if ( pos != std::string::npos ) { // We found the token
        ...
        if (verbose ) {
            std::cout << std::setw(20) << token << " " << value << std::endl;
        }
        if (ss.bad()) {
...
template<typename T>
int BOVCollection::_readMetadata( const std::string & token, std::string &line, std::vector<T> &value, bool verbose) {
    // Skip comments
    if (line[0] == '#') {
        return 0;
    }
