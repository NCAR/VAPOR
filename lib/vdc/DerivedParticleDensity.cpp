...
DerivedParticleDensity::DerivedParticleDensity(string varName, DC *dc, string meshName, DataMgr *dataMgr)
: DerivedDataVar(varName), _dc(dc), _meshName(meshName), _dataMgr(dataMgr) {}
...
int DerivedParticleDensity::Initialize()
{
    ...
    for (auto dv : dataVarNames) {
        ...
        if (STLUtils::Contains(coordNames, string("Position_x")) &&
            STLUtils::Contains(coordNames, string("Position_y")) &&
            STLUtils::Contains(coordNames, string("Position_z"))) {
            dataVar = dv;
            break;
        }
    }
...
//    printf("Read(%s, %s, %s)\n", C(_derivedVarName), C(min), C(max));
    ...
    for (int z = 0; z < ozd; z++)
        for (int y = 0; y < oyd; y++)
            for (int x = 0; x < oxd; x++)
                region[z*oyd*oxd + y*oxd + x] = data[(min[2]+z)*yd*xd + (min[1]+y)*xd + (min[0]+x)];
...
int DerivedParticleDensity::ReadRegionBlock(int fd, const std::vector <size_t> &min, const std::vector <size_t> &max, float *region)
{
...
bool DerivedParticleDensity::VariableExists(size_t ts, int reflevel, int lod) const
{
    return true;
}
...
vector<string> DerivedParticleDensity::GetInputs() const
{
    return {_dataVar};
}
...
DerivedParticleAverage::DerivedParticleAverage(string varName, DC *dc, string meshName, DataMgr *dataMgr, string inputVar)
: DerivedParticleDensity(varName, dc, meshName, dataMgr)
{
    _dataVar = inputVar;
}
