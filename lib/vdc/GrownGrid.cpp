...
GrownGrid::GrownGrid( const VAPoR::Grid* gp, VAPoR::DataMgr* mp, float z)
            : _grid2d( gp ), _dataMgr( mp ), _defaultZ( z ),
              _minZ( z > 0.0f ? z * 0.99999f : z * 1.00001f ),
...
//
float GrownGrid::GetValueNearestNeighbor( const std::vector <double> &coords) const
{
    return 0.0f;
}
    
float GrownGrid::GetValueLinear( const std::vector <double> &coords) const 
{
    return 0.0f;
}
