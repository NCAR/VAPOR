...
int NetCDFCollection::GetAttType(
    string varname, string attname) const {
...
void NetCDFCollection::GetAtt(
    string varname, string attname, std::vector<double> &values) const {
    values.clear();
...
void NetCDFCollection::GetAtt(
    string varname, string attname, std::vector<long> &values) const {
    values.clear();
...
int NetCDFCollection::GetTimes(
    string varname, vector<double> &times) const {
    map<string, TimeVaryingVar>::const_iterator p = _variableList.find(varname);
    if (p == _variableList.end()) {
...
int NetCDFCollection::GetFile(
    size_t ts, string varname, string &file, size_t &local_ts) const {
...
bool NetCDFCollection::_GetVariableInfo(
    string varname, NetCDFSimple::Variable &varinfo) const {
...
bool NetCDFCollection::GetMissingValue(
    string varname, double &mv) const {
...
int NetCDFCollection::_get_var_index(
    const vector<NetCDFSimple::Variable> variables, string varname) const {
...
bool tvmap_cmp(
    ...
    NetCDFCollection::TimeVaryingVar::tvmap_t b) { return (a._time < b._time); };
} // namespace VAPoR
...
bool NetCDFCollection::TimeVaryingVar::GetMissingValue(
    string attname, double &mv) const {
    mv = 0.0;
