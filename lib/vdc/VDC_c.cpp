...
VDC *VDC_new()
{ VDC_DEBUG_called(); return new VAPoR::VDCNetCDF(); }
...
{ VDC_DEBUG_called(); return p->GetDimension(string(dimname), *dimension); }

void VDC_GetDimensionNames(const VDC *p, char ***names, int *count)
{ VDC_DEBUG_called(); _stringVectorToCStringArray(p->GetDimensionNames(), names, count); }

int  VDC_GetCoordVarInfo(const VDC *p, const char *varname, VDCCoordVar *var)
{ VDC_DEBUG_called(); return p->GetCoordVarInfo(string(varname), *var); }

int  VDC_GetDataVarInfo(const VDC *p, const char *varname, VDCDataVar *var)
{ VDC_DEBUG_called(); return p->GetDataVarInfo(string(varname), *var); }

int  VDC_GetBaseVarInfo(const VDC *p, const char *varname, VDCBaseVar *var)
{ VDC_DEBUG_called(); return p->GetBaseVarInfo(string(varname), *var); }

void VDC_GetDataVarNames(const VDC *p, char ***names, int *count)
{ VDC_DEBUG_called(); _stringVectorToCStringArray(p->GetDataVarNames(), names, count); }

void VDC_GetCoordVarNames(const VDC *p, char ***names, int *count)
{ VDC_DEBUG_called(); _stringVectorToCStringArray(p->GetCoordVarNames(), names, count); }

int  VDC_GetNumRefLevels(const VDC *p, const char *varname)
{ VDC_DEBUG_called(); return p->GetNumRefLevels(string(varname)); }
...
{ VDC_DEBUG_called(); _stringVectorToCStringArray(p->GetAttNames(string(varname)), names, count); }

int VDC_GetAttType(const VDC *p, const char *varname, const char *attname)
{ VDC_DEBUG_called(); return _XTypeToInt(p->GetAttType(string(varname), string(attname))); }

int  VDC_VariableExists(const VDC *p, size_t ts, const char *varname, int reflevel, int lod)
{ VDC_DEBUG_called(); return p->VariableExists(ts, string(varname), reflevel, lod); }

int  VDC_IsTimeVarying(const VDC *p, const char *varname)
{ VDC_DEBUG_called(); return p->IsTimeVarying(string(varname)); }
...
{ VDC_DEBUG_printff("(%s)\n", p?"*p":"NULL"); return p->EndDefine(); }

int VDC_PutVar(VDC *p, const char *varname, int lod, const float *data)
{ VDC_DEBUG_called(); return p->PutVar(string(varname), lod, data); }
