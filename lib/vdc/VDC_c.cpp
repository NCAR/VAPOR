...
{
    VDC_DEBUG_printff("(%s, \"%s\", <dims>, %li, <coords>, %li, \"%s\", %s, %s)\n", p ? "*p" : "NULL", varname, dimnamesCount, coordvarsCount, units, _XTypeToString(_IntToXType(xtype)),
...
{
    VDC_DEBUG_printff("(%s, \"%s\", %s, %li, \"%s\", \"%s\", %s, %s, %s)\n", p ? "*p" : "NULL", varname, _stringVectorToString(_strArrayToStringVector(dimnames, dimnamesCount)).c_str(), dimnamesCount,
