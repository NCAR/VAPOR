...
NetCDFCFCollection::~NetCDFCFCollection()
{
    ...
    for (itr = _derivedVarsMap.begin(); itr != _derivedVarsMap.end(); ++itr) { delete itr->second; }
    _derivedVarsMap.clear();
}
...
int NetCDFCFCollection::_parse_formula(string formula_terms, map<string, string> &parsed_terms) const
{
...
int NetCDFCFCollection::InstallStandardVerticalConverter(string cvar, string newvar, string units)
{
    ...
    if (standard_name.empty()) { varinfo.GetAtt("long_name", standard_name); }
    ...
    } else if ((standard_name.compare("altitude") == 0) || (standard_name.compare("model_level_number") == 0) || (_udunit->IsLengthUnit(units))) {    // noop
...
void NetCDFCFCollection::InstallDerivedCoordVar(string varname, DerivedVar *derivedVar, int axis)
{
    ...
    case 1: _latCoordVars.push_back(varname); break;
    case 2: _vertCoordVars.push_back(varname); break;
    case 3: _timeCoordVars.push_back(varname); break;
    default: break;
    }
...
int NetCDFCFCollection::DerivedVar_ocean_s_coordinate_g1::ReadSlice(float *slice, int)
{
    ...
    for (size_t y = 0; y < ny; y++) {
        for (size_t x = 0; x < nx; x++) {
            slice[y * nx + x] = _depth_c * _s[z] + (_depth[y * nx + x] - _depth_c) * _C[z] + _eta[y * nx + x] * (1 + (_depth_c * _s[z] + (_depth[y * nx + x] - _depth_c) * _C[z]) / _depth[y * nx + x]);
        }
    }
...
NetCDFCFCollection::DerivedVar_ocean_s_coordinate_g2::DerivedVar_ocean_s_coordinate_g2(NetCDFCFCollection *ncdfcf, const std::map<string, string> &formula_map) : DerivedVar(ncdfcf)
{
    _dims.resize(3);
    _dimnames.resize(3);
    _s = NULL;
...
int NetCDFCFCollection::DerivedVar_ocean_s_coordinate_g2::Read(float *buf, int)
{
    ...
    for (size_t z = 0; z < nz; z++) {
        for (size_t y = 0; y < ny; y++) {
            for (size_t x = 0; x < nx; x++) {
                buf[z * nx * ny + y * nx + x] = _eta[y * nx + x] + (_eta[y * nx + x] + _depth[y * nx + x]) * ((_depth_c * _s[z] + _depth[y * nx + x] * _C[z]) / (_depth_c + _depth[y * nx + x]));
            }
        }
    }
    return (0);
...
int NetCDFCFCollection::DerivedVar_ocean_s_coordinate_g2::ReadSlice(float *slice, int fd)
{
    ...
    for (size_t y = 0; y < ny; y++) {
        for (size_t x = 0; x < nx; x++) {
            slice[y * nx + x] = _eta[y * nx + x] + (_eta[y * nx + x] + _depth[y * nx + x]) * ((_depth_c * _s[z] + _depth[y * nx + x] * _C[z]) / (_depth_c + _depth[y * nx + x]));
        }
    }
...
NetCDFCFCollection::DerivedVar_AHSPC::DerivedVar_AHSPC(NetCDFCFCollection *ncdfcf, const std::map<string, string> &formula_map) : DerivedVar(ncdfcf)
{
    _dims.resize(3);
    _dimnames.resize(3);
...
int NetCDFCFCollection::DerivedVar_AHSPC::Open(size_t ts)
{
    ...
    if (_ncdfc->VariableExists("Z3")) {
        ...
        if (_ncdfc->VariableExists("PHIS")) {
            ...
            if (fd < 0) {
                for (size_t i = 0; i < nx * ny; i++) { PHIS[i] = 0.0; }
            }
...
int NetCDFCFCollection::DerivedVar_AHSPC::Read(float *buf, int)
{
    ...
    int rc;
    while ((rc = NetCDFCFCollection::DerivedVar_AHSPC::ReadSlice(ptr, 0)) > 0) { ptr += _dims[0] * _dims[1]; }
    return (rc);
}
...
int NetCDFCFCollection::DerivedVar_AHSPC::CalculateElevation()
{
    ...
    for (size_t NL = 0; NL < NLAT; NL++) {
        ...
        int rc = NetCDFCFCollection::DerivedVar_AHSPC::DCZ2(PS1,        // surface pressure
                                               ...
                                               float **PTERM, float **ZSLICE)
{
    ...
    for (size_t i = 0; i < IDIM; i++) { PMLN[i] = new float[KMAX + 1]; }
    ...
    for (size_t K = 0; K < KMAX - 2; K++) {
        for (size_t L = K + 1; L < KMAX - 1; L++) {
            for (size_t I = 0; I < IMAX; I++) { ZSLICE[I][K] = ZSLICE[I][K] + PTERM[I][L]; }
        }
    }

    if (PMLN) {
        for (size_t i = 0; i < IDIM; i++) { delete[] PMLN[i]; }
        delete[] PMLN;
    }
    return (0);
}
...
NetCDFCFCollection::DerivedVar_noop::DerivedVar_noop(NetCDFCFCollection *ncdfcf, const std::map<string, string> &formula_map, string units) : DerivedVar(ncdfcf)
{
    _zvar.clear();
    _native_units.clear();
...
int NetCDFCFCollection::DerivedVar_noop::ReadSlice(float *slice, int fd) { return (Read(slice, fd)); }

int NetCDFCFCollection::DerivedVar_noop::SeekSlice(int offset, int whence, int fd) { return (0); }

NetCDFCFCollection::DerivedVarTime::DerivedVarTime(NetCDFCFCollection *ncdfcf, string native_var, string native_units, string derived_units) : DerivedVar(ncdfcf)
{
...
NetCDFCFCollection::DerivedVarTimeFromMem::DerivedVarTimeFromMem(string name, string units, string timeDimName, const std::vector<double> times) : DerivedVar(NULL)
{
    _ts = 0;
