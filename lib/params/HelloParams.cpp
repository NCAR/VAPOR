...
Validate(int type){
	...
	if (!dataMgr) return;
	if(dataMgr->GetDataVarNames(3,true).size() == 0) return;
	...
	if (doOverride) {
		...
		pt2 = GetPoint2();
		for (int i = 0; i<3; i++){
			if (pt1[i] < minExts[i]) pt1[i] = minExts[i];
			if (pt2[i] < minExts[i]) pt2[i] = minExts[i];
			if (pt1[i] > maxExts[i]) pt1[i] = maxExts[i];
			if (pt2[i] > maxExts[i]) pt2[i] = maxExts[i];
		}
		SetPoint1(pt1);
		SetPoint2(pt2);
	}
