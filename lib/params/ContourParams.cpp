...
void ContourParams::GenerateContourValues(double start, double spacing, int num, Contours *c) {
    if (!c)
        c = GetCurrentContours();

    vector<double> vals;
    for (int i = 0; i < num; i++)
        vals.push_back(start + i * spacing);
    c->SetContourValues(vals);
}
...
void ContourParams::SetContourCount(int num) {
    GenerateContourValues(GetContourMin(), GetContourSpacing(), num);
}

void ContourParams::SetContourMin(double val) {
    GenerateContourValues(val, GetContourSpacing(), GetContourCount());
}

void ContourParams::SetContourSpacing(double val) {
    GenerateContourValues(GetContourMin(), val, GetContourCount());
}
