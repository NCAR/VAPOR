...
MapperFunction::MapperFunction(
    ParamsBase::StateSave *ssave) : ParamsBase(ssave, MapperFunction::GetClassType()),
                                    _numEntries(256) {
...
MapperFunction::MapperFunction(
    const MapperFunction &rhs) : ParamsBase(rhs),
                                 _numEntries(256) {
...
int MapperFunction::LoadFromFile(string path, vector<double> defaultDataBounds) {
    ...
    if ((newMin < defaultDataBounds[0]) ||
        (newMin > defaultDataBounds[1])) {
        newMin = defaultDataBounds[0];
    }
    if ((newMax < defaultDataBounds[0]) ||
...
//
//----------------------------------------------------------------------------
void MapperFunction::DeleteOpacityMap(const OpacityMap *omap) {

    m_opacityMaps->Remove(omap);
}
