...
{ assert(d.size() >= 2); return vec2(d[0], d[1]); }
...
template <float vec2::*primaryAxis, float vec2::*secondaryAxis>
class Stack : public Div {
...
struct HStack : public Stack<&vec2::x, &vec2::y> {using Stack::Stack;};
struct VStack : public Stack<&vec2::y, &vec2::x> {using Stack::Stack;};
...
void ColorbarRenderer::Render(GLManager *glm, RenderParams *rp)
{
    ...
    colorbar.Draw = [&]() {
        DrawColorBar(lgl, mf, vec2(0), colorbarSize);
    };
    ...
    auto valueForTick = [&](int i) -> float {
        return mfRange.x+(mfRange.y-mfRange.x)*i/(tickCount-1.f);
    };
    auto textForTick = [&](int i) -> string {
        return formatTick(valueForTick(i));
    };
    auto tickTextSize = [&](int i) -> vec2 {
        return tickFont.GetFont()->TextDimensions(textForTick(i));
    };
    Div tickLabels;
    for (int i = 0; i < tickCount; i++) {
        tickLabels.Size = glm::max(tickLabels.Size, tickTextSize(i));
    }
    tickLabels.Draw = [&]() {
        float h = colorbarSize.y;
        lgl->Color(foregroundColor);
    ...
    title.Draw = [&]() {
        titleFont.DrawText(vec2(0), titleText);
    };
