...
ShaderMgr::~ShaderMgr()
{
    ...
    for (std::map<std::string, ShaderProgram *>::const_iterator iter = _effects.begin(); iter != _effects.end(); ++iter) delete iter->second;
}
...
std::string ShaderMgr::_convertDefines(std::string defines)
{
    ...
    if (defines[defines.length() - 1] != ';') { return result; }
...
{
    SetDiagMsg("ShaderMgr::defineEffect(%s,%s,%s)", baseName.c_str(), defines.c_str(), instanceName.c_str());
...
int ShaderMgr::UploadEffectData(std::string effect, std::string variable, int value)
{
    ...
    if (current == _effects[effect]->GetProgram()) {
        ...
        if (_effects[effect]->Enable() < 0) return -1;
        if (GLEW_VERSION_2_0) {
            glUniform1i(_effects[effect]->UniformLocation(variable), value);
        } else {
            glUniform1iARB(_effects[effect]->UniformLocation(variable), value);
...
// Uploads data to the selected effect's uniform
//----------------------------------------------------------------------------
int ShaderMgr::UploadEffectData(std::string effect, std::string variable, int value1, int value2, int value3)
{
    // Check to see if program is currently loaded
    GLint current;
    glGetIntegerv(GL_CURRENT_PROGRAM, &current);
...
// Uploads data to the selected effect's uniform
//----------------------------------------------------------------------------
int ShaderMgr::UploadEffectData(std::string effect, std::string variable, int value1, int value2, int value3, int value4)
{
    // Check to see if program is currently loaded
    GLint current;
    glGetIntegerv(GL_CURRENT_PROGRAM, &current);
...
// Uploads data to the selected effect's uniform
//----------------------------------------------------------------------------
int ShaderMgr::UploadEffectData(std::string effect, std::string variable, float value1, float value2)
{
    // Check to see if program is currently loaded
    GLint current;
    glGetIntegerv(GL_CURRENT_PROGRAM, &current);
...
// Uploads data to the selected effect's uniform
//----------------------------------------------------------------------------
int ShaderMgr::UploadEffectData(std::string effect, std::string variable, float value1, float value2, float value3)
{
    // Check to see if program is currently loaded
    GLint current;
    glGetIntegerv(GL_CURRENT_PROGRAM, &current);
...
// Uploads data to the selected effect's uniform
//----------------------------------------------------------------------------
int ShaderMgr::UploadEffectData(std::string effect, std::string variable, float value1, float value2, float value3, float value4)
{
    // Check to see if program is currently loaded
    GLint current;
    glGetIntegerv(GL_CURRENT_PROGRAM, &current);
...
void ShaderMgr::PrintEffects()
{
    ...
    for (std::map<std::string, ShaderProgram *>::const_iterator iter = _effects.begin(); iter != _effects.end(); ++iter) {
...
{
    for (std::map<std::string, ShaderProgram *>::const_iterator iter = _effects.begin(); iter != _effects.end(); ++iter) {
        if (iter->second->GetProgram() == prog) return iter->first;
    }
    return std::string("");
}
...
bool ShaderMgr::CheckFramebufferStatus()
{
    ...
    case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT: std::cout << "[ERROR] Framebuffer incomplete: Attachment is NOT complete." << std::endl; return false;

    case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT: std::cout << "[ERROR] Framebuffer incomplete: No image is attached to FBO." << std::endl; return false;

    case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER: std::cout << "[ERROR] Framebuffer incomplete: Draw buffer." << std::endl; return false;

    case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER: std::cout << "[ERROR] Framebuffer incomplete: Read buffer." << std::endl; return false;

    case GL_FRAMEBUFFER_UNSUPPORTED: std::cout << "[ERROR] Framebuffer incomplete: Unsupported by FBO implementation." << std::endl; return false;
...
int ShaderMgr::_getEfcFiles(string dirname, vector<string> &efcFiles)
{
    ...
    while ((ent = readdir(dir)) != NULL) {
        ...
        if (ext.compare(".efc") == 0) { efcFiles.push_back(file); }
    }
    closedir(dir);
    return (0);
}
