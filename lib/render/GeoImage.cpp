...
GeoImage::GeoImage(int pixelsize, int nbands) : _pixelsize(pixelsize),
                                                _nbands(nbands) {
    assert(pixelsize = 8);
    assert(nbands = 4);
    _tif = NULL;
...
GeoImage::GeoImage() : _pixelsize(8),
                       _nbands(4) {
    _path.clear();
    _tif = NULL;
}
...
int GeoImage::TiffGetImageDimensions(
    int dirnum, size_t &width, size_t &height) const {
    assert(_tif != NULL);
    width = 0;
    height = 0;
...
int GeoImage::TiffReadImage(
    int dirnum, unsigned char *texture) const {
    assert(_tif != NULL);
