...
int ContourRenderer::edgeCode(int i, int j, float isoval, float *dataVals)
{
    ...
    if ((dataVals[i + _gridSize * j] < isoval && dataVals[i + 1 + _gridSize * j] >= isoval) || (dataVals[i + _gridSize * j] >= isoval && dataVals[i + 1 + _gridSize * j] < isoval))
        intersectionCode += 1;
    // check for crossing between (i+1,j+1) and (i+1,j)
    if ((dataVals[i + 1 + _gridSize * j] < isoval && dataVals[i + 1 + _gridSize * (j + 1)] >= isoval) || (dataVals[i + 1 + _gridSize * j] >= isoval && dataVals[i + 1 + _gridSize * (j + 1)] < isoval))
        intersectionCode += 2;
    ...
    case (15):    // disambiguate 7 and 8, based on whether or not average is on same side of isovalue as (i,j) vertex.
        ...
        if (((dataVals[i + _gridSize * j] < isoval) && (avgvalue < isoval)) || ((dataVals[i + _gridSize * j] > isoval) && (avgvalue > isoval))) {
...
void ContourRenderer::buildEdges(int iso, float *dataVals, float mv)
{
    ...
    for (int i = 0; i < _gridSize - 1; i++) {
        for (int j = 0; j < _gridSize - 1; j++) {
            ...
            if ((dataVals[i + j * _gridSize] == mv) || (dataVals[i + 1 + j * _gridSize] == mv) || (dataVals[i + (j + 1) * _gridSize] == mv) || (dataVals[i + 1 + (j + 1) * _gridSize] == mv))
                cellCase = 0;
            else
