...
int ContourRenderer::performRendering(size_t timestep, DataMgr *dataMgr)
{
    ...
    for (int iso = 0; iso < cParams->GetContourCount(); iso++) {
        ...
        for (int linenum = 0; linenum < lines.size(); linenum++) {
            ...
            if (mapToTerrain) {
                if (std::isnan(pointa[0]) || std::isnan(pointa[1]) || std::isnan(pointb[0]) || std::isnan(pointb[1])) {
                    dontDraw = true;
                } else {
...
int ContourRenderer::buildLineCache(DataMgr *dataMgr)
{
    ...
    _objectNums.clear();
    for (int iso = 0; iso < isovals.size(); iso++) { buildEdges(iso, dataVals, mv); }
...
int ContourRenderer::edgeCode(int i, int j, float isoval, float *dataVals)
{
    ...
    if ((dataVals[i + _sampleSize * j] < isoval && dataVals[i + 1 + _sampleSize * j] >= isoval) || (dataVals[i + _sampleSize * j] >= isoval && dataVals[i + 1 + _sampleSize * j] < isoval))
        intersectionCode += 1;
    // check for crossing between (i+1,j+1) and (i+1,j)
    if ((dataVals[i + 1 + _sampleSize * j] < isoval && dataVals[i + 1 + _sampleSize * (j + 1)] >= isoval)
        || (dataVals[i + 1 + _sampleSize * j] >= isoval && dataVals[i + 1 + _sampleSize * (j + 1)] < isoval))
        intersectionCode += 2;
    // check for crossing between (i,j+1) and (i+1,j+1)
    if ((dataVals[i + _sampleSize * (j + 1)] < isoval && dataVals[i + 1 + _sampleSize * (j + 1)] >= isoval)
        || (dataVals[i + _sampleSize * (j + 1)] >= isoval && dataVals[i + 1 + _sampleSize * (j + 1)] < isoval))
        intersectionCode += 4;
    // check for crossing between (i,j+1) and (i,j)
    if ((dataVals[i + _sampleSize * j] < isoval && dataVals[i + _sampleSize * (j + 1)] >= isoval) || (dataVals[i + _sampleSize * j] >= isoval && dataVals[i + _sampleSize * (j + 1)] < isoval))
        intersectionCode += 8;
    ...
    case (0): ecode = 0; break;
    case (3): ecode = 2; break;
    case (5): ecode = 5; break;
    case (6): ecode = 3; break;
    case (9): ecode = 1; break;
    case (10): ecode = 6; break;
    case (12): ecode = 4; break;
    case (15):    // disambiguate 7 and 8, based on whether or not average is on same side of isovalue as (i,j) vertex.
        ...
        if (((dataVals[i + _sampleSize * j] < isoval) && (avgvalue < isoval)) || ((dataVals[i + _sampleSize * j] > isoval) && (avgvalue > isoval))) {
...
void ContourRenderer::traverseCurves(int iso)
{
    ...
    for (edgeEdgeIter = _edgeEdge1.begin(); edgeEdgeIter != _edgeEdge1.end(); edgeEdgeIter++) {
        ...
        if (_markerBit[startingEdge]) continue;    // already marked; keep looking
        ...
        if (edgeEdgeTestIter == _edgeEdge2.end()) firstDirection = false;
...
void ContourRenderer::buildEdges(int iso, float *dataVals, float mv)
{
    ...
    for (int j = 0; j < _sampleSize - 1; j++) {
        for (int i = 0; i < _sampleSize - 1; i++) {
            ...
            if ((dataVals[i + j * _sampleSize] == mv) || (dataVals[i + 1 + j * _sampleSize] == mv) || (dataVals[i + (j + 1) * _sampleSize] == mv) || (dataVals[i + 1 + (j + 1) * _sampleSize] == mv)) {
                cellCase = 0;
            } else
    ...
    if (cParams->GetTextDensity() > 0. && cParams->GetTextEnabled()) { traverseCurves(iso); }
...
bool ContourRenderer::cacheIsValid(int timestep)
{
    ...
    if (it == cacheKeys.end()) return false;
    cacheKey *     thisKey = it->second;
    ContourParams *cParams = (ContourParams *)GetActiveParams();
    if (thisKey->hgtVar != cParams->GetHeightVariableName()) return false;
    if (thisKey->varname != cParams->GetVariableName()) return false;
    if (thisKey->lod != cParams->GetCompressionLevel()) return false;
    if (thisKey->refLevel != cParams->GetRefinementLevel()) return false;
...
void ContourRenderer::updateCacheKey(int timestep)
{
    ...
    int dimSize = minExt.size();
    for (int i = 0; i < minExt.size(); i++) { extents.push_back(minExt[i]); }
    for (int i = 0; i < maxExt.size(); i++) { extents.push_back(maxExt[i]); }
