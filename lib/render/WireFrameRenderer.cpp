...
WireFrameRenderer::WireFrameRenderer(
	...
	WireFrameRenderer::GetClassType(), instName, dataMgr), _drawList(0)
{}
...
void WireFrameRenderer::_drawCell(
	...
	glDrawElements(
		GL_LINE_LOOP, count, GL_UNSIGNED_INT, indices1
	);
	...
	glDrawElements(
		GL_LINE_LOOP, count, GL_UNSIGNED_INT, indices1+count
	);
	...
	glDrawElements(
		GL_LINES, 2*count, GL_UNSIGNED_INT, indices2
	);
...
int WireFrameRenderer::_buildCache()
{
	...
	for (
		;
		itr != end_itr && coord_itr != end_coord_itr; 
		++itr, ++coord_itr, ++index
	) {
