...
//	if (!viz->projectPointToWin(boxCtr, winCoords)) return false;
	if (!projectPointToWin(boxCtr, winCoords)) {cout << "A" << endl; return false;}
	boxCtr[handleNum] += 0.1f;
//	if (!viz->projectPointToWin(boxCtr, dispCoords)) return false;
	if (!projectPointToWin(boxCtr, dispCoords)) {cout << "B" << endl; return false;}
	...
	if (vecNorm == 0.f) {cout << "C" << endl; return false;}
...
rayHandleIntersect(double ray[3], 
