...
int VolumeOSPRay::LoadData(const Grid *grid)
{
    ...
    if       (p->GetValueLong("osp_test_volume", false))   volume = _loadVolumeTest(grid);
    else if  (p->GetValueLong("osp_force_regular", false)) volume = _loadVolumeRegular(grid);
    else if (dynamic_cast<const RegularGrid *>     (grid)) volume = _loadVolumeRegular(grid);
    else if (dynamic_cast<const StructuredGrid *>  (grid)) volume = _loadVolumeStructured(grid);
    else if (dynamic_cast<const UnstructuredGrid *>(grid)) volume = _loadVolumeUnstructured(grid);
...
float VolumeOSPRay::GuestimateFastModeSpeedupFactor() const
{
    return 5;
}
...
void VolumeOSPRay::_setupRenderer(bool fast)
{
    ...
    if (fast) {
        ...
        vec3 dir(ddir[0], ddir[1], ddir[2]);
//        printf("OSPDepthFromGLPerspective(%f, %f, %f, %f, (%f, %f, %f), (%f, %f, %f), %p, %i, %i)\n", glm::degrees(mm->FOV), mm->Aspect, mm->Near, mm->Far, dir.x,dir.y,dir.z, up.x,up.y,up.z, _depthData.data(), fbSize.x, fbSize.y);
...
static WindingOrder GetWindingOrderTetra(const vec3 &a, const vec3 &b, const vec3 &c, const vec3 &d)
{
...
OSPVolume VolumeOSPRay::_loadVolumeStructured(const Grid *grid)
{
    ...
    typedef struct {
        union {
            struct {unsigned int i0, i1, i2, i3, i4, i5, i6, i7; };
            unsigned int i[8];
        };
    } Cell;
...
#define FACE(a, b, c) { vec3 n = glm::normalize(glm::cross(testCell[b]-testCell[a], testCell[c]-testCell[a])); lgl->Normal3fv((float*)&n); lgl->Vertex(testCell[a]); lgl->Vertex(testCell[b]); lgl->Vertex(testCell[c]); }
#define QUAD(a, b, c, d) { FACE(a, b, d); FACE(d, b, c); }
