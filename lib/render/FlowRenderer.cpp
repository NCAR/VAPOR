...
int FlowRenderer::_renderAdvection(const flow::Advection *adv) {
    ...
    if (_renderStatus != FlowStatus::UPTODATE) {
        ...
        if (!_cache_isSteady) {
            ...
            if (int(_cache_currentTS) - _cache_pastNumOfTimeSteps > 0)
                startingTime = _timestamps[_cache_currentTS - _cache_pastNumOfTimeSteps];
        }
...
int FlowRenderer::_renderFromAnAdvectionLegacy(const flow::Advection *adv,
                                               FlowParams *params,
                                               bool fast) {
...
void FlowRenderer::_particleHelper1(std::vector<float> &vec,
