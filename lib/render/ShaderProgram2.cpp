...
void ShaderProgram2::UnBind() {
    glUseProgram(0);
}

void ShaderProgram2::AddShader(Shader *s) {
...
template <typename T>
bool ShaderProgram2::SetUniform(const std::string name, T value) const {
    if (!IsBound()) {
...
SmartShaderProgram::SmartShaderProgram(ShaderProgram2 *program)
    : _program(program) {
    printf("Auto Bind\n");
    _program->Bind();
}
