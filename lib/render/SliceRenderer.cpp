...
std::vector<double> SliceRenderer::_calculateDeltas(
) const {
...
inline void SliceRenderer::_getInlineJSampleCoordinates(
    std::vector<double> &coords,
...
void SliceRenderer::_getJSampleCoordinates(
    std::vector<double> &coords,
    const std::vector<double> deltas,
    const int j
) const {
...
inline void SliceRenderer::_getInlineISampleCoordinates(
    std::vector<double> &coords,
    const std::vector<double> deltas,
    const int i 
) const {
...
void SliceRenderer::_getISampleCoordinates(
    std::vector<double> &coords,
    const std::vector<double> deltas,
    const int i 
) const {
...
int SliceRenderer::_saveTextureData(int method = 0) {
    ...
    if (method == 0 || method == 1) deltas = _calculateDeltas();
    for (int j=0; j<_textureHeight; j++) {

        if (method == 0) _getJSampleCoordinates(coords, deltas, j);
        else if (method == 1) _getInlineJSampleCoordinates(coords, deltas, j);

        for (int i=0; i<_textureWidth; i++) {

            if (method == 0) _getISampleCoordinates(coords, deltas, i);
            else if (method == 1) _getInlineISampleCoordinates(coords, deltas, i);
            else if (method == 2) _getSampleCoordinates(coords, i, j);
    ...
    if (method == 0)      cout << "New way         " << duration << endl;
    else if (method == 1) cout << "New with inline " << duration << endl;
    else if (method == 2) cout << "Old way         " << duration << endl; 
