...
void SliceRenderer::_getModifiedExtents(vector<double> &min, vector<double> &max) const
{
    ...
    switch (box->GetOrientation()) {
        case XY: min[Z] = sampleLocation[Z]; max[Z] = sampleLocation[Z]; break;
        case XZ: min[Y] = sampleLocation[Y]; max[Y] = sampleLocation[Y]; break;
        case YZ: min[X] = sampleLocation[X]; max[X] = sampleLocation[X]; break;
        default: VAssert(0);
    }
}
