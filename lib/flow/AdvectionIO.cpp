  ...
  for (size_t s_idx = 0; s_idx < adv->GetNumberOfStreams(); s_idx++) {
    ...
    for (const auto& p : stream) {
      ...
      if (!p.IsSpecial()) {
        ...
        std::fprintf(f, "%lu, %f, %f, %f, %.4d-%.2d-%.2d_%.2d:%.2d:%.2d, %f", s_idx, cX, cY,
                     p.location.z, year, month, day, hour, minute, second, p.time);
...
auto flow::InputSeedsCSV(const std::string& filename) -> std::vector<flow::Particle>
{
  ...
  for (std::string line; std::getline(ifs, line);) {
    ...
    line.erase(
        std::remove_if(line.begin(), line.end(), [](unsigned char c) { return std::isspace(c); }),
        line.end());
