...
OptionParser::OptDescRec_T	set_opts[] = {
	{"nts",		1, 	"1","Number of timesteps to play"},
...
RenderParams *CreateRender(
	ControlExec *CE, string winName, string className, string name
) {
...
int CreateNew(ControlExec *CE, string winName) {
	...
	if (opt.arrow) {
		...
		myRParams = (ArrowParams *) CreateRender(
			CE, winName, ArrowParams::m_classTag, name
		);
	...
	if (opt.twoD) {
		...
		myRParams = (TwoDDataParams *) CreateRender(
			CE, winName, TwoDDataParams::m_classTag, name
		);
	...
	if (opt.hello) {
		...
		myRParams = (HelloParams *) CreateRender(
			CE, winName, HelloParams::m_classTag, name
		);
...
int main(int argc, char **argv) {
	...
	if (useSession){
		...
		CE->ActivateRender(
			winName, HelloParams::m_classTag, "hello1", true
		);
	}
#endif
