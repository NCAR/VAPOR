...
OptionParser::OptDescRec_T set_opts[] = {{"savefile", 1, "", "Path to output file"}, {"help", 0, "", "Print this message and exit"}, {NULL}};

OptionParser::Option_T get_options[] = {{"save", Wasp::CvtToString, &opt.savefile, sizeof(opt.savefile)}, {"help", Wasp::CvtToBoolean, &opt.help, sizeof(opt.help)}, {NULL}};
...
int main(int argc, char **argv)
{
    ...
    size_t size;
    for (int i = 0; i < 3; i++) regmax[i] = (bdim[i] << refinelevel) - 1;

    size_t vdim[3];    // dimension in voxels of resampled grid

    for (int i = 0; i < 3; i++) vdim[i] = (regmax[i] - regmin[i] + 1) * celldim[i];
    ...
    if (strlen(opt.savefile)) {
        ...
        if (!fp) { cerr << "Can't open output file " << opt.savefile << endl; }
    ...
    for (int z = 0; z < vdim[2]; z++) {
        for (int y = 0; y < vdim[1]; y++) {
            for (int x = 0; x < vdim[0]; x++) {
                ...
                if (grid[srcindex] < cmin) cmin = grid[srcindex];
                if (grid[srcindex] > cmax) cmax = grid[srcindex];
            }
        }
    }
    ...
    for (int z = 0; z < vdim[2]; z++) { bufwriter.WriteSlice(&grid[z * vdim[1] * vdim[0]]); }
    if (bufwriter.GetErrCode()) {
        cerr << ProgName << " : " << bufwriter.GetErrMsg() << endl;
        exit(1);
    }
