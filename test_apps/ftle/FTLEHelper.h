...
public:
    ...
    double operator[](const size_t component) const { return vector[component]; }
    ...
    Vec3 operator*(const Vec3 &vec) const { return Vec3(this->vector[0] * vec[0], this->vector[1] * vec[1], this->vector[2] * vec[2]); }
...
std::ostream &operator<<(std::ostream &strm, const Vec3 &vec) { return strm << "{" << vec[0] << "," << vec[1] << ", " << vec[2] << "}"; }

inline Vec3 GetVec3(const std::vector<flow::Particle> &data, const long long int index) { return Vec3{data.at(index).location.x, data.at(index).location.y, data.at(index).location.z}; }
...
template<int dimensions> class DimensionHelper;

template<> class DimensionHelper<2> : public FTLECalculator {
public:
...
template<> class DimensionHelper<3> : public FTLECalculator {
public:
    DimensionHelper() = default;
...
void CalculateFTLE(const std::vector<flow::Particle> &startPositions, const std::vector<flow::Particle> &endPositions, const GridMetaData &metaData, const double duration, std::vector<double> &output)
{
