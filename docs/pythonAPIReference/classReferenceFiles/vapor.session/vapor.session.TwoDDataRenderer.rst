.. _vapor.session.TwoDDataRenderer:


vapor.session.TwoDDataRenderer
------------------------------


Help on class TwoDDataRenderer in vapor.session:

vapor.session.TwoDDataRenderer = class TwoDDataRenderer(Renderer)
 |  vapor.session.TwoDDataRenderer(renderParams: cppyy.gbl.VAPoR.RenderParams, id: str)
 |  
