.. _vapor.session.ContourRenderer:


vapor.session.ContourRenderer
-----------------------------


Help on class ContourRenderer in vapor.session:

vapor.session.ContourRenderer = class ContourRenderer(Renderer)
 |  vapor.session.ContourRenderer(renderParams: cppyy.gbl.VAPoR.RenderParams, id: str)
 |  
