.. _vapor.session.PythonDataset:


vapor.session.PythonDataset
---------------------------


Help on class PythonDataset in vapor.session:

vapor.session.PythonDataset = class PythonDataset(Dataset)
 |  vapor.session.PythonDataset(dataMgr: cppyy.gbl.VAPoR.DataMgr, id: str, ses)
 |  
